...
Процедура мСцен_ПреобразоватьВДеревоСценарияНаКлиенте(Знач ТекстСценария,
	...
	// перенесем в таблицу	
	Для каждого стр из РезультатПреобразования["Значение"] Цикл		
		...
		стр_н.UID = XMLСтрока(новый УникальныйИдентификатор());
	...
	Для каждого стр из ТаблицаДанныхСценария Цикл
		...
		Если стр.ТипОбъекта="Window" Тогда
			Если ИмяОкна<>стр.ЗаголовокОбъекта Тогда
				ИмяОкна = стр.ЗаголовокОбъекта;
				ЭлементОкно = ДеревоСценария.ПолучитьЭлементы().Добавить();
		...
		Если стр.Действие="НайтиОбъект" Тогда
			Если ИмяОбъекта<>стр.ЗаголовокОбъекта Тогда
				ИмяОбъекта = стр.ЗаголовокОбъекта;
...
Функция ПреобразоватьНаименованиеДействия(Знач Наименование)
	...
	Если Наименование="find window" Тогда
