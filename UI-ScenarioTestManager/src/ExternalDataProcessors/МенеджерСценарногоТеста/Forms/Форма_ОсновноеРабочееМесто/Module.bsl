...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// браузеры
	стр_н = ТаблицаДоступныхБраузеров.Добавить();
	стр_н.Выбрана = Истина;
	стр_н.Представление = "IE";
	
	стр_н = ТаблицаДоступныхБраузеров.Добавить();
	стр_н.Представление = "Chrome";
	
	стр_н = ТаблицаДоступныхБраузеров.Добавить();
	стр_н.Представление = "Firefox";
	
	стр_н = ТаблицаДоступныхБраузеров.Добавить();
	стр_н.Представление = "Opera";
...
&НаКлиенте
Процедура мСценСкрипт_ЗапуститьБраузер(Команда)
	
	ТекущийБраузер = "";
	
	Для каждого стр из ТаблицаДоступныхБраузеров Цикл
		Если стр.Выбрана=Истина Тогда
			ТекущийБраузер = стр.Представление;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Модуль_ЗаписьЖурналаДействий.ЗапуститьБраузер_Selenium_API(ТекущийБраузер,Объект.НомерПортаExternAutomationUI);
КонецПроцедуры

&НаКлиенте
Процедура мСценСкрипт_ЗакрытьБраузер(Команда)
	ТекущийБраузер = "";
	...
	Модуль_ЗаписьЖурналаДействий.ЗакрытьБраузер_Selenium_API(ТекущийБраузер,Объект.НомерПортаExternAutomationUI);
...
Процедура мСценСкрипт_ОбработатьТипУзла(Узел,ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,мПараметрыКлиентСервер)
	...
	// определеим ПИД
	Если ТекущийAPI = "AutomationUI" Тогда
		Если НЕ ПуллТестируемыхПриложений=Неопределено Тогда
			PID = ПуллТестируемыхПриложений.Получить(ТекущийНомерПорта);
		Иначе // порт по умолчанию
	...
	мПараметры.Вставить("ИмяОбъекта",Узел.ИмяОбъекта);
	...
	мПараметры.Вставить("ПутьКФайлуExternAutomationUI",Объект.ПутьКФайлуExternAutomationUI);
	...
	ИначеЕсли Узел.Действие = "Команда" ИЛИ Узел.Действие = "Command" Тогда
		...
		ТекущийМодульОбработки.мСценСкрипт_ВыполнитьКоманду(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная,мПараметры);
...
Процедура ЗагрузитьСценарийФорматXMLСвойстваШага(ЧтениеXML,ШагСвойства)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		ИначеЕсли Найти("Наименование Описание Library ПутьБлокШагов ИмяПараметра ИмяПараметра2 Area УсловиеСравнения
			...
			| ClickY ClickX xPath ИмяКлассаОбъекта API",ИмяТега)					
			И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
			ЧтениеXML.Прочитать();
...
Процедура мСценЖурнал_НачатьЗапись(Команда)	
	...
	ИначеЕсли Объект.ИспользуемыйAPI="AutomationUI" Тогда
		...
		Для каждого стр из ТаблицаДоступныхПриложений Цикл
			...
			КонецЕсли;				
		КонецЦикла;
		Модуль_ЗаписьЖурналаДействий.НачатьЗапись_AutomationUI_API(PID,Объект.НомерПортаExternAutomationUI);
	...
	ИначеЕсли Объект.ИспользуемыйAPI="Selenium" Тогда
		Модуль_ЗаписьЖурналаДействий.НачатьЗапись_Selenium_API(Объект.НомерПортаExternAutomationUI);
		Элементы.мСценЖурнал_ЗавершитьЗапись.Доступность = Истина;
		Элементы.мСценЖурнал_ПрерватьЗапись.Доступность = Истина;
		Элементы.мСценЖурнал_НачатьЗапись.Видимость = Ложь;
		Элементы.мСценЖурнал_ПаузаЗапись.Видимость = Истина;
...
Процедура мСценЖурнал_ПаузаЗапись(Команда)
	...
	Если Объект.ИспользуемыйAPI="1cClientApp" Тогда
		...
		КонецПопытки;	
	ИначеЕсли Объект.ИспользуемыйAPI="AutomationUI" Тогда
		Модуль_ЗаписьЖурналаДействий.ПаузаЗапись_AutomationUI_API(Объект.НомерПортаExternAutomationUI);
	КонецЕсли;	
...
Процедура мСценЖурнал_ЗавершитьЗапись(Команда)
	Если Объект.ИспользуемыйAPI="1cClientApp" Тогда
		...
		КонецПопытки;	
	ИначеЕсли Объект.ИспользуемыйAPI="AutomationUI" Тогда
		ИсточникТекст = Модуль_ЗаписьЖурналаДействий.ЗавершитьЗапись_AutomationUI_API(Объект.НомерПортаExternAutomationUI);
	ИначеЕсли Объект.ИспользуемыйAPI="Selenium" Тогда
		ИсточникТекст = Модуль_ЗаписьЖурналаДействий.ЗавершитьЗапись_Selenium_API(Объект.НомерПортаExternAutomationUI);
	КонецЕсли;	
...
Процедура мСценСкрипт_ОбновитьСписокДоступныхПриложений(Команда)
	...
	СтруктураОтвета = Модуль_ЗаписьЖурналаДействий.СписокПриложенийДляОтладки_AutomationUI_API(Объект.НомерПортаExternAutomationUI);
...
&НаКлиенте
Процедура ТаблицаДосупныхБраузеровВыбранаПриИзменении(Элемент)
	ТекущиеДанные = Элементы.ТаблицаДоступныхБраузеров.ТекущиеДанные;
