...
&НаКлиенте
Перем Модуль_ФункцияПамять;
&НаКлиенте
Перем Модуль_ВыгрузитьЗагрузитьФорматПроектаСценарияXML;
&НаКлиенте
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонструкторHTML = ПолучитьМакетНаСервере("Editor").ПолучитьТекст();
	//DialogBoxHTML = ПолучитьМакетНаСервере("Chat").ПолучитьТекст(); 
...
Процедура ИнициализироватьСлужебныеНастройки() Экспорт
	...
	// Модуль загрузки/выгрузки сценария
	Модуль_ВыгрузитьЗагрузитьФорматПроектаСценарияXML = ПолучитьФорму("ВнешняяОбработка.МенеджерСценарногоТеста.Форма.Модуль_ВыгрузитьЗагрузитьФорматПроектаСценарияXML");
	// Объявление модулей
	...
	// Другое
	Модуль_ФункцияПамять = ПолучитьФорму("ВнешняяОбработка.МенеджерСценарногоТеста.Форма.Модуль_ФункцияПамять"); 
...
&НаСервере
Функция ПолучитьHMLТекстМакета(ИмяМакета)
...
&НаКлиенте
Функция ПолучитьТаблицуЗамеров() Экспорт
	МассивСтруктур = Новый Массив;
	
	Для каждого стр из ТаблицаЗамеров Цикл
		Если стр.Зафиксирован=Ложь Тогда
			Продолжить;
		КонецЕсли;
		СтруктураЗаписи = Новый Структура();
		СтруктураЗаписи.Вставить("Key",стр.Ключ);
		СтруктураЗаписи.Вставить("Duration",стр.Длительность);
		СтруктураЗаписи.Вставить("Date",стр.ДатаНачала);
		СтруктураЗаписи.Вставить("Id",стр.Идентификатор);
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ РежимМенеджераТестированияДоступен() И (Объект.ИспользуемыйAPI="1cClientApp" ИЛИ Объект.ИспользуемыйAPI="") Тогда
	...
	Элементы.РазделыСправки.СписокВыбора.Добавить("Help","Помощь");
	//Элементы.РазделыСправки.СписокВыбора.Добавить("QuickStart","Быстрый старт");
	Элементы.РазделыСправки.СписокВыбора.Добавить("HelpSteps","Описание шагов");
	Элементы.РазделыСправки.СписокВыбора.Добавить("HelpCommands","Описание команд");
	Элементы.РазделыСправки.СписокВыбора.Добавить("HelpCI","Интеграция CI");
	//Элементы.РазделыСправки.СписокВыбора.Добавить("HelpАdvice","Советы разработки");
	Элементы.РазделыСправки.СписокВыбора.Добавить("HelpArticles","Статьи, уроки");
	Элементы.РазделыСправки.СписокВыбора.Добавить("HelpWinAutomationUI","WinAutomationUI");
	Если НЕ ЗначениеЗаполнено(РазделыСправки) Тогда
		РазделыСправки="Help";
	КонецЕсли;
	AboutHTML = ПолучитьHMLТекстМакета(РазделыСправки);	
...
// Процедура - Выделение и замена в тексте сценария шаблонов из библиотеки
//
// Параметры:
//  мПараметр	 - 	Массив структур - входящие данные с разметкой замены шаблонов
//
&НаКлиенте
Процедура ВыделениеИЗаменаВСценарииШаблонов(Знач мПараметр)
	
	КешСоответсивяНумерацииУзламДерева = Новый Соответствие();
	СформироватьКешСтруктурДереваСценария(ДеревоСценария,1,мПараметр.ПреобразовыватьБлокиШагов,КешСоответсивяНумерацииУзламДерева);
	
	//ПреобразовыватьБлокиШагов
	// 1. это эквивалентно вставке в позицию блока библиотеки
	Для каждого шаблон из мПараметр.МассивСтруктурЗамен Цикл
		
		Если НЕ ЗначениеЗаполнено(шаблон.ПутьШаблон) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ шаблон.Выбрана Тогда
			Продолжить;
		КонецЕсли;
		
		УзелВхода = КешСоответсивяНумерацииУзламДерева.Получить(шаблон.Начало);
		Если УзелВхода=Неопределено Тогда
			// почему-то не нашелся узел входа, возможно другой сценарий
		ИначеЕсли УзелВхода.ПолучитьРодителя()=Неопределено Тогда
			Узел = ДеревоСценария.ПолучитьЭлементы().Вставить(ДеревоСценария.ПолучитьЭлементы().Индекс(УзелВхода));
		Иначе
			Узел = УзелВхода.ПолучитьРодителя().ПолучитьЭлементы().Вставить(УзелВхода.ПолучитьРодителя().ПолучитьЭлементы().Индекс(УзелВхода));
		КонецЕсли;
		
		Если Узел=Неопределено Тогда
			Сообщить("Не смогли вставить шаблоне в позицию "+шаблон.Начало);
			Продолжить;
		КонецЕсли;
		
		Узел.Активность = Истина;
		Узел.Действие = "ГотовыйБлокШагов";
		Узел.ДанныеКартинки = Модуль_ПреобразоватьЖурналДействийВДеревоСценария.мСцен_ПолучитьДанныеКартинки_НаКлиенте(новый Структура("Действие,Команда,ТипОбъекта",Узел.Действие,Узел.Команда,Узел.ТипОбъекта));
		Узел.Наименование = шаблон.Шаблон;
		Узел.ПутьБлокШагов = ЭтотОбъект.Объект.ПутьККаталогуБиблиотекиСценариев+"\"+шаблон.ПутьШаблон;
		Узел.Library = "ПутьККаталогуБиблиотекиСценариев";
		Узел.Наименование = "Библиотека """+Узел.Наименование+"""";
		Узел.UID = Строка(новый UUID());
		Узел.ЭтоБлок = Истина;
		
		// подгружаем файл блока шагов
		мОписаниеСценария = мСцен_ПолучитьСтруктуруОписанияСценария();
		Если НЕ Модуль_ВыгрузитьЗагрузитьФорматПроектаСценарияXML.ЗагрузитьСценарийФорматXML(мОписаниеСценария, Узел.ПутьБлокШагов,Истина) = Ложь Тогда
			мСценСкрипт_РазвернутьТаблицуВДерево(Истина,Узел);
		КонецЕсли;
		
	КонецЦикла;
	// 2. потом снять выделение с шагов		
	// развернуть
	Для каждого шаблон из мПараметр.МассивСтруктурЗамен Цикл
		...
		Для ш=шаблон.Начало по шаблон.Окончание Цикл
			
			УзелВхода = КешСоответсивяНумерацииУзламДерева.Получить(ш);	
			// если все подчиненные
			Если УзелВхода=Неопределено Тогда
			ИначеЕсли УзелВхода.ЭтоБлок=Истина И УзелВхода.Действие="ГотовыйБлокШагов" Тогда
				УзелВхода.Активность = Ложь;
				ПройтиПоУзлуУстновкаЭлементаВниз(УзелВхода,"Активность",Ложь);
			ИначеЕсли УзелВхода.ПолучитьЭлементы().Количество()=0 Тогда
				УзелВхода.Активность = Ложь;
				СнятьФлагИерархияВверх(УзелВхода,"Активность",Ложь);
				РазвернутьДоТекущегоУзлаИерархия(УзелВхода);
			КонецЕсли;
...
Функция мСцен_ПолучитьНаименованиеПоТегу(ИмяТега)
	...
	ИначеЕсли ИмяТега = "FormItemAddition" Then
...
&НаКлиенте
Процедура СформироватьКешСтруктурДереваСценария(Знач УзелРодитель, Позиция=1, ПреобразовыватьБлокиШагов=Ложь, КешСоответсивяНумерацииУзламДерева)
	
	Для каждого Узел из УзелРодитель.ПолучитьЭлементы() Цикл
		
		// в кеш
		КешСоответсивяНумерацииУзламДерева.Вставить(Позиция,Узел);
		Позиция = Позиция + 1;
		
		// в иерархию
		Если Узел.ЭтоБлок=Истина и Узел.Действие="ГотовыйБлокШагов" Тогда
			Если ПреобразовыватьБлокиШагов=Истина Тогда
				СформироватьКешСтруктурДереваСценария(Узел, Позиция, ПреобразовыватьБлокиШагов, КешСоответсивяНумерацииУзламДерева);
			КонецЕсли;
		Иначе
			СформироватьКешСтруктурДереваСценария(Узел, Позиция, ПреобразовыватьБлокиШагов, КешСоответсивяНумерацииУзламДерева);
		КонецЕсли;
...
&НаКлиенте
Процедура мСценСкрипт_РазвернутьДеревоВТаблицу(ВсеУзлы=Ложь,ВсеБлоки=Истина) Экспорт
...
Процедура мСценСкрипт_ОбработатьТипУзла(Узел,ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,мПараметрыКлиентСервер)  Экспорт
	...
	Area				= Узел.Area;
	CommandRef			= Узел.CommandRef;
	ИмяКлассаОбъекта	= Узел.ИмяКлассаОбъекта;
	FileName 			= Узел.FileName;
	mKey 				= Узел.Key;
	xPath				= Узел.xPath;
	...
	мСценСкрипт_ПодставитьЗначениеПараметра(Area);
	мСценСкрипт_ПодставитьЗначениеПараметра(CommandRef);
	мСценСкрипт_ПодставитьЗначениеПараметра(ИмяКлассаОбъекта);
	мСценСкрипт_ПодставитьЗначениеПараметра(FileName);
	мСценСкрипт_ПодставитьЗначениеПараметра(mKey);
	Если ЗначениеЗаполнено(xPath) И Найти(xPath,"&") Тогда
		мСценСкрипт_ПодставитьЗначениеПараметра(xPath);
	КонецЕсли;
	...
	мПараметры.Вставить("ИмяКлассаОбъекта",ИмяКлассаОбъекта);
	мПараметры.Вставить("ИспользоватьВариантыПоиска",Узел.ИспользоватьВариантыПоиска);
	...
	мПараметры.Вставить("CommandRef",CommandRef);
	...
	мПараметры.Вставить("xPath",xPath);
	мПараметры.Вставить("Интервал",Узел.Интервал);
	// для НайтиФорму
	мПараметры.Вставить("ЗаголовокОбъекта",ЗаголовокОбъекта);
	мПараметры.Вставить("ИспользоватьВариантыПоиска",Узел.ИспользоватьВариантыПоиска);
	...
	мПараметры.Вставить("Key",mKey);
	мПараметры.Вставить("Type",Узел.Type);
	мПараметры.Вставить("FileName",FileName);
	мПараметры.Вставить("TurnToCommentInBlock",Узел.TurnToCommentInBlock);
	
	// параметры подключения по умолчанию
	ПараметрыПодключенияПоУмолчанию = новый Структура();
	ПараметрыПодключенияПоУмолчанию.Вставить("ПутьКФайлуСценария",Объект.ПутьКФайлуСценария);
	ПараметрыПодключенияПоУмолчанию.Вставить("СтрокаПодключения",Объект.ПутьПодключенияКлиентаТестирования);
	ПараметрыПодключенияПоУмолчанию.Вставить("ДополнительныеОпцииЗапуска",Объект.ДополнительныеОпцииЗапуска);
	ПараметрыПодключенияПоУмолчанию.Вставить("ЭтоФайловаяБаза",Объект.ФайловаяБаза);
	ПараметрыПодключенияПоУмолчанию.Вставить("Пользователь1С",Объект.Пользователь1С);
	ПараметрыПодключенияПоУмолчанию.Вставить("Пароль1С",Объект.Пароль1С);
	ПараметрыПодключенияПоУмолчанию.Вставить("НомерПорта",Объект.НомерПорта);
	ПараметрыПодключенияПоУмолчанию.Вставить("ПутьКИсполняемомуФайлу1С",Объект.ПутьКИсполняемомуФайлу1С);
	мПараметры.Вставить("ПараметрыПодключенияПоУмолчанию",ПараметрыПодключенияПоУмолчанию);
	...
	ИначеЕсли Узел.Действие="ЗначениеВПараметр" Тогда
		
		мСценСкрипт_ЗначениеВПараметр(ОписаниеОшибки,Узел.ПредставлениеДанных,Узел.ИмяПараметра);
	...
	ИначеЕсли Узел.Действие="ПолучитьАктивноеОкно" ИЛИ Узел.Действие="GetActiveWindow" Тогда
		
		ТекущийМодульОбработки.мСценСкрипт_ПолучитьАктивноеОкно(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная,мПараметры);
	...
	ИначеЕсли Узел.Действие = "ПолучитьКомандныйИнтерфейс" ИЛИ Узел.Действие = "GetCommandInterface" Тогда
		...
		ТекущийМодульОбработки.мСценСкрипт_ПолучитьКомандныйИнтерфейс(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная,мПараметры);
	...
	ИначеЕсли Узел.Действие = "Замер" ИЛИ Узел.Действие = "Measurement" Тогда
		
		// посчитаем замер
		Если Узел.Команда="Начать" Тогда
			стр_н = ТаблицаЗамеров.Добавить();
			стр_н.Идентификатор = Строка(Новый UUID());
			стр_н.Ключ = mKey;
			стр_н.ДатаНачала = ТекущаяДата();
			стр_н.ДатаНачалаУнив = ТекущаяУниверсальнаяДатаВМиллисекундах();
		ИначеЕсли Узел.Команда="Завершить" Тогда
			мОтбор = Новый Структура("Ключ,Зафиксирован",mKey,Ложь);
			н_строки = ТаблицаЗамеров.НайтиСтроки(мОтбор);
			Если н_строки.Количество()>0 Тогда
				н_строки[0].Длительность = Окр((ТекущаяУниверсальнаяДатаВМиллисекундах()-н_строки[0].ДатаНачалаУнив)/1000,2);
				н_строки[0].Зафиксирован=Истина;
			КонецЕсли;
		ИначеЕсли Узел.Команда="Удалить"  Тогда
			мОтбор = Новый Структура("Ключ,Зафиксирован",mKey,Ложь);
			н_строки = ТаблицаЗамеров.НайтиСтроки(мОтбор);
			Если н_строки.Количество()>0 Тогда
				ТаблицаЗамеров.Удалить(н_строки[0]);
			КонецЕсли;
		КонецЕсли;
		
	ИначеЕсли Узел.Действие = "Память" ИЛИ Узел.Действие = "Memory" Тогда
		
		
		Модуль_ФункцияПамять.мСценСкрипт_ОбработатьОперациюРаботыПамяти(ОписаниеОшибки,мПараметрыКлиент,мПараметры);
...
&НаКлиенте
Процедура мСценСкрипт_ЗначениеВПараметр(ОписаниеОшибки,Знач ЗначениеДанных,Знач ИмяПараметра)
	
	мСцен_УстановитьЗначениеПараметра(ИмяПараметра,ЗначениеДанных);	
...
&НаКлиенте
Процедура ПройтиПоУзлуПолучитьВсеПодключенияИзБазы(УзелРодитель)
	// Идем далее по узлу
	Узлы = УзелРодитель.ПолучитьЭлементы();
	
	Для каждого Узел из Узлы Цикл
		Если (Узел.Действие="Память" ИЛИ Узел.Действие="Memory") 
		И (нрег(Узел.Команда)="вспомнить" И Узел.Type="1CConnectionProperty" ) Тогда
			// вызовем подключения
			ОписаниеОшибки = "";
			мПараметрыКлиент = новый Структура;
			мПараметры = новый Структура();
			...
			Модуль_ФункцияПамять.мСценСкрипт_ОбработатьОперациюРаботыПамяти(ОписаниеОшибки,мПараметрыКлиент,мПараметры);
		КонецЕсли;
		ПройтиПоУзлуПолучитьВсеПодключенияИзБазы(Узел);
	КонецЦикла;
...
&НаКлиенте
Процедура СнятьФлагИерархияВверх(УзелРебенок,Знач Имя,Знач Значение)
	...
	// если у родителя все сняты, то и родителя выключаем
	ВсеВыключены = Истина;
	Для каждого уз_с из Узел.ПолучитьЭлементы() Цикл
		Если уз_с[Имя]=Истина Тогда
			ВсеВыключены = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ВсеВыключены=Истина Тогда
		Узел[Имя] = Значение;
		СнятьФлагИерархияВверх(Узел,Имя,Значение);
	КонецЕсли;
		
КонецПроцедуры

&НаКлиенте
Процедура РазвернутьДоТекущегоУзлаИерархия(УзелРебенок)
	
	УзелРодитель = УзелРебенок.ПолучитьРодителя();
	
	Если УзелРодитель=Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ДеревоСценария.Развернуть(УзелРодитель.ПолучитьИдентификатор(),Ложь);
	
	РазвернутьДоТекущегоУзлаИерархия(УзелРодитель);	
...
//
&НаКлиенте
Процедура мСценСкрипт_СбросПараметровПередЗапускомСценария(ЗапускDDT=Ложь)  Экспорт
	ИндексТекущегоШага = 0;
	ИндексПереходСТекущегоШага = 0;
	...
	Если Объект.DataDrivenTest=Истина И ЗапускDDT=Ложь Тогда
		...
		Если НЕ ТаблицаDDT.Количество()=0 Тогда
			УстновитьПараметрыИзDDT(ИндексТекущегоШагаDDT);
		Иначе
			ИнформацияОбОшибке = "Не удалось загрузить данные из файла (количество=0) '"+Объект.ПутьКФайлуДанных+"'. Сценарий выполняться не будет!"; 
			СообщитьОбОшибке("ЗагрузкаДанныхDDTИзФайла",ИнформацияОбОшибке);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Функция мСцен_ПолучитьСтруктуруОписанияСценария()
	мОписаниеСценария = Новый Структура();
	
	мОписаниеСценария.Вставить("ДеревоСценария",ДеревоСценария);
	мОписаниеСценария.Вставить("ТаблицаСценария",ТаблицаСценария);
	мОписаниеСценария.Вставить("ПараметрыСценария",ПараметрыСценария);
	
	// версии
	мОписаниеСценария.Вставить("ТаблицаПоддерживаемыеВерсииТК",ТаблицаПоддерживаемыеВерсииТК);
	
	// логика
	мОписаниеСценария.Вставить("Логика_Action",Логика_Action);
	мОписаниеСценария.Вставить("Логика_Description",Логика_Description);
	мОписаниеСценария.Вставить("Логика_Preconditions",Логика_Preconditions);
	мОписаниеСценария.Вставить("Логика_Effects",Логика_Effects);
	мОписаниеСценария.Вставить("Логика_Cost",Логика_Cost);
	
	// объект
	мОписаниеСценария.Вставить("ПутьККаталогуБиблиотекиСценариев",Объект.ПутьККаталогуБиблиотекиСценариев);
	мОписаниеСценария.Вставить("ПутьКФайлуСценария",Объект.ПутьКФайлуСценария);
	
	мОписаниеСценария.Вставить("Наименование",Объект.Наименование);
	мОписаниеСценария.Вставить("Автор",Объект.Автор);
	мОписаниеСценария.Вставить("АдресИнтернет",Объект.АдресИнтернет);
	мОписаниеСценария.Вставить("Браузер",Объект.Браузер);
	мОписаниеСценария.Вставить("НомерПорта",Объект.НомерПорта);
	мОписаниеСценария.Вставить("ИспользуемыйAPI",Объект.ИспользуемыйAPI);
	мОписаниеСценария.Вставить("ИдентификаторТеста",Объект.ИдентификаторТеста);
	мОписаниеСценария.Вставить("Severity",Объект.Severity);
	
	
	мОписаниеСценария.Вставить("ВерсияФайлаСценария",ВерсияФайлаСценария);
	//мОписаниеСценария.Вставить("",Объект.);
	
	Возврат мОписаниеСценария;
КонецФункции

&НаКлиенте
Процедура мСцен_ОбработатьСтруктуруОписанияСценария(мОписаниеСценария)
	
	//ДеревоСценария = мОписаниеСценария.ДеревоСценария;
	//ТаблицаСценария = мОписаниеСценария.ТаблицаСценария;
	//ПараметрыСценария = мОписаниеСценария.ПараметрыСценария;
	
	// версии
	//ТаблицаПоддерживаемыеВерсииТК = мОписаниеСценария.ТаблицаПоддерживаемыеВерсииТК;
	
	// логика
	Логика_Action = мОписаниеСценария.Логика_Action;
	Логика_Description = мОписаниеСценария.Логика_Description;
	//Логика_Preconditions = мОписаниеСценария.Логика_Preconditions;
	//Логика_Effects = мОписаниеСценария.Логика_Effects;
	Логика_Cost = мОписаниеСценария.Логика_Cost;
	
	// объект
	//ПутьККаталогуБиблиотекиСценариев = мОписаниеСценария.ПутьККаталогуБиблиотекиСценариев;
	//ПутьКФайлуСценария = мОписаниеСценария.ПутьКФайлуСценария;
	
	Объект.Наименование = мОписаниеСценария.Наименование;
	Объект.Автор = мОписаниеСценария.Автор;
	Объект.АдресИнтернет = мОписаниеСценария.АдресИнтернет;
	Объект.Браузер = мОписаниеСценария.Браузер;
	Объект.ИспользуемыйAPI = мОписаниеСценария.ИспользуемыйAPI;
	Объект.ИдентификаторТеста = мОписаниеСценария.ИдентификаторТеста;
	Объект.Severity = мОписаниеСценария.Severity;
	
	
	ВерсияФайлаСценария = мОписаниеСценария.ВерсияФайлаСценария;
...
&НаКлиенте
Процедура мСценСкрипт_ЗагрузитьСценарийПоПути(ПутьКФайлу) Экспорт
	
	// загружаем
	мОписаниеСценария = мСцен_ПолучитьСтруктуруОписанияСценария();
	Если НЕ Модуль_ВыгрузитьЗагрузитьФорматПроектаСценарияXML.ЗагрузитьСценарийФорматXML(мОписаниеСценария, ПутьКФайлу) = Ложь Тогда
		мСцен_ОбработатьСтруктуруОписанияСценария(мОписаниеСценария);
...
Процедура мСценСкрипт_СохранитьСценарий(Команда)
	...
	Если Объект.ТипФайлаСценария="XML" Тогда
		мОписаниеСценария = мСцен_ПолучитьСтруктуруОписанияСценария();
		ТекстоваяСтрокаФайла = Модуль_ВыгрузитьЗагрузитьФорматПроектаСценарияXML.ВыгрузитьСценарийФорматXML(мОписаниеСценария);
		ЗаполнитьЗначенияСвойств(Объект,мОписаниеСценария);
	Иначе
...
Процедура НаименованиеПриИзменении(Элемент)
	...
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.Логика_Action) Тогда
		Объект.Логика_Action = Объект.Наименование;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ПерезаполнитьТаблицуКомандПоAPI(Действие="Команда")
	...
	Если Действие="Команда" Тогда  	
		...
		КонецЕсли;
	ИначеЕсли Действие="Память" Тогда
		ДоступныеКомандыМассив = мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандПамять();
	ИначеЕсли Действие="Замер" Тогда
		ДоступныеКомандыМассив = мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандЗамер();
	КонецЕсли;
...
Процедура ТаблицаДействияПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные.Действие = "Команда" Тогда
		...
		ПерезаполнитьТаблицуКомандПоAPI(ТекущиеДанные.Действие);
	ИначеЕсли ТекущиеДанные.Действие = "Память" Тогда
		Элементы.ТаблицаКоманды.Видимость = Истина;
		// Перезаполним таблицу команд по API
		ПерезаполнитьТаблицуКомандПоAPI(ТекущиеДанные.Действие);
	ИначеЕсли ТекущиеДанные.Действие = "Замер" Тогда
		Элементы.ТаблицаКоманды.Видимость = Истина;
		// Перезаполним таблицу команд по API
		ПерезаполнитьТаблицуКомандПоAPI(ТекущиеДанные.Действие);
	Иначе
...
Процедура ТаблицаСвойстваПриАктивизацииСтроки(Элемент)
	...
	ТекущиеДанныеСценарий = Элементы.ДеревоСценария.ТекущиеДанные;
	
	Если ТекущиеДанные=Неопределено Тогда
		Возврат;	
	КонецЕсли; 
	
	Если ТекущиеДанныеСценарий=Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Попытка
		Действие = Элементы.ДеревоСценария.ТекущиеДанные.Действие;
	Исключение
	КонецПопытки;
	
	Попытка
		Команда = ТекущиеДанные.Свойство;
	Исключение
	КонецПопытки;
	...
	мСценРедактор_ИзменитьСписокВыбораПриИзмененииСтроки(Элементы.ТаблицаСвойстваЗначение,Действие,Команда);
КонецПроцедуры
...
Процедура ДеревоСценарияПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока=Истина Тогда
		мСценРедактор_ДеревоСценарияПриНачалеРедактированияФрагмент(Элемент, Копирование);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия="ПроставитьШаблоныЗамены" Тогда
		Если ТипЗнч(Параметр)=Тип("Структура") Тогда
			
			ВыделениеИЗаменаВСценарииШаблонов(Параметр);
...
Функция мСценРедактор_ПолучитьМассивСтруктурДоступныхДействий()
	...
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Получить активное окно");
	стр_н.Вставить("Действие","ПолучитьАктивноеОкно");
	стр_н.Вставить("ДанныеКартинки",мСцен_ПолучитьДанныеКартинки_НаКлиенте(стр_н));
	ДоступныеДействияМассив.Добавить(стр_н);
	// НайтиФорму
	стр_н = новый Структура;
	...
	// ЗначениеВПараметр
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","значение->параметр");
	стр_н.Вставить("Действие","ЗначениеВПараметр");
	стр_н.Вставить("ДанныеКартинки",мСцен_ПолучитьДанныеКартинки_НаКлиенте(стр_н));
	ДоступныеДействияМассив.Добавить(стр_н);	
	
	// Память
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Память");
	стр_н.Вставить("Действие","Память");
	стр_н.Вставить("ДанныеКартинки",мСцен_ПолучитьДанныеКартинки_НаКлиенте(стр_н));
	ДоступныеДействияМассив.Добавить(стр_н);	
	
	// Замер
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Замер");
	стр_н.Вставить("Действие","Замер");
	стр_н.Вставить("ДанныеКартинки",мСцен_ПолучитьДанныеКартинки_НаКлиенте(стр_н));
	ДоступныеДействияМассив.Добавить(стр_н);	
	...
	// Логика
//	стр_н = новый Структура;
//	стр_н.Вставить("Синоним","Логика");
//	стр_н.Вставить("Действие","Логика");
//	стр_н.Вставить("ДанныеКартинки",мСцен_ПолучитьДанныеКартинки_НаКлиенте(стр_н));
//	ДоступныеДействияМассив.Добавить(стр_н);	
...
&НаКлиенте
Функция мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандПамять()
	
	ДоступныеКомандыМассив = новый Массив;  	
	
	// УстановитьМаксимальноеВремяВыполненияДействия
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Запомнить/Сохранить");
	стр_н.Вставить("Команда","Запомнить");
	ДоступныеКомандыМассив.Добавить(стр_н); 	
	
	// ПолучитьПредставлениеДанных
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Вспомнить/Загрузить");
	стр_н.Вставить("Команда","Вспомнить");
	ДоступныеКомандыМассив.Добавить(стр_н); 	
	// ПолучитьТекстРедактирования
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Забыть/Удалить");
	стр_н.Вставить("Команда","Забыть");
	ДоступныеКомандыМассив.Добавить(стр_н); 
...
Функция мСценРедактор_ПолучитьМассивСтруктурДоступныхКоманд()
	...
	ДоступныеКомандыМассив.Добавить(стр_н);
	// ПолучитьАдресТекущейОбласти
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","ПолучитьАдресТекущейОбласти");
	стр_н.Вставить("Команда","ПолучитьАдресТекущейОбласти");
	ДоступныеКомандыМассив.Добавить(стр_н);
	// ПолучитьПолеТекущейОбласти
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","ПолучитьПолеТекущейОбласти");
	стр_н.Вставить("Команда","ПолучитьПолеТекущейОбласти");
	ДоступныеКомандыМассив.Добавить(стр_н);	
	// ПолучитьТекстОбласти
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","ПолучитьТекстОбласти");
	стр_н.Вставить("Команда","ПолучитьТекстОбласти");
	ДоступныеКомандыМассив.Добавить(стр_н);		
	...
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","ПолучитьТекущуюИнформациюОбОшибке");
	стр_н.Вставить("Команда","ПолучитьТекущуюИнформациюОбОшибке");
	ДоступныеКомандыМассив.Добавить(стр_н); 	
	...
	// НажатьНаЭлементСостоянияПросмотра 
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","НажатьНаЭлементСостоянияПросмотра");
	стр_н.Вставить("Команда","НажатьНаЭлементСостоянияПросмотра");
	ДоступныеКомандыМассив.Добавить(стр_н); 
	// ПолучитьТекстыЭлементовСостоянияПросмотра
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","ПолучитьТекстыЭлементовСостоянияПросмотра");
	стр_н.Вставить("Команда","ПолучитьТекстыЭлементовСостоянияПросмотра");
	ДоступныеКомандыМассив.Добавить(стр_н); 
	// УдалитьЭлементСостоянияПросмотра 
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","УдалитьЭлементСостоянияПросмотра");
	стр_н.Вставить("Команда","УдалитьЭлементСостоянияПросмотра");
	ДоступныеКомандыМассив.Добавить(стр_н); 
...
Функция мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандAutomationUI()
	...
	// Выбрано?
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Выбран элемент?");
	стр_н.Вставить("Команда","is selected");
	ДоступныеКомандыМассив.Добавить(стр_н);
...
Функция мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандSelenium()
	...
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Получить адрес");
	стр_н.Вставить("Команда","get url");
	ДоступныеКомандыМассив.Добавить(стр_н);
	...
	// get xpath
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Получить xPath");
	стр_н.Вставить("Команда","get xpath");
	ДоступныеКомандыМассив.Добавить(стр_н);

	// get tag
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Получить tag");
	стр_н.Вставить("Команда","get tag");
	ДоступныеКомандыМассив.Добавить(стр_н);
	
	// get displayed
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Получить видимость");
	стр_н.Вставить("Команда","get displayed");
	ДоступныеКомандыМассив.Добавить(стр_н);
	
	// get enabled 
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Получить enabled");
	стр_н.Вставить("Команда","get enabled");
	ДоступныеКомандыМассив.Добавить(стр_н);
	
	// wait until disappears 
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Ждать пока не скроется");
	стр_н.Вставить("Команда","wait until disappears");
	ДоступныеКомандыМассив.Добавить(стр_н);	
	
	// get css
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Получить атрибут класса");
	стр_н.Вставить("Команда","get css");
	ДоступныеКомандыМассив.Добавить(стр_н);
	
	// get children count 
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Количество потомков");
	стр_н.Вставить("Команда","get children count");
	ДоступныеКомандыМассив.Добавить(стр_н);	
	
	// is element stale
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Элемент устарел?");
	стр_н.Вставить("Команда","is element stale");
	ДоступныеКомандыМассив.Добавить(стр_н);
...
&НаКлиенте
Функция мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандЗамер()
	
	ДоступныеКомандыМассив = новый Массив;  	
	
	// УстановитьМаксимальноеВремяВыполненияДействия
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Начать");
	стр_н.Вставить("Команда","Начать");
	ДоступныеКомандыМассив.Добавить(стр_н); 	
	
	// ПолучитьПредставлениеДанных
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Завершить");
	стр_н.Вставить("Команда","Завершить");
	ДоступныеКомандыМассив.Добавить(стр_н); 	
	
	// ПолучитьТекстРедактирования
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Удалить");
	стр_н.Вставить("Команда","Удалить");
	ДоступныеКомандыМассив.Добавить(стр_н); 
...
Функция мСценРедактор_ПолучитьМассивСтруктурДоступныхСвойств()
	...
	стр_н.Вставить("Синоним","Один уровень вниз");
	стр_н.Вставить("Значение",Неопределено);
	стр_н.Вставить("ОписаниеТипов",ОписаниеТиповБулево);
	стр_н.Вставить("ТолькоПросмотр",Истина);
	стр_н.Вставить("Видимость",Ложь);
	...
	стр_н = новый Структура;
	стр_н.Вставить("Свойство","TurnToCommentInBlock");
	стр_н.Вставить("Синоним","В блоке как комментарий");
	стр_н.Вставить("Значение",Неопределено);
	стр_н.Вставить("ОписаниеТипов",ОписаниеТиповБулево);
	стр_н.Вставить("ТолькоПросмотр",Истина);
	стр_н.Вставить("Видимость",Ложь);
	...
	// Goal
//	стр_н = новый Структура;
//	стр_н.Вставить("Свойство","Goal");
//	стр_н.Вставить("Синоним","Цель");
//	стр_н.Вставить("Значение",Неопределено);
//	стр_н.Вставить("ОписаниеТипов",ОписаниеТиповСтрока);
//	стр_н.Вставить("ТолькоПросмотр",Ложь);
//	стр_н.Вставить("Видимость",Ложь);	
//	ДоступныеСвойстваМассив.Добавить(стр_н);
	...
	стр_н.Вставить("Синоним","Имя файла/Путь к файлу");
	стр_н.Вставить("Значение","");
	стр_н.Вставить("ОписаниеТипов",ОписаниеТиповСтрока);
	стр_н.Вставить("ТолькоПросмотр",Истина);
	стр_н.Вставить("Видимость",Ложь);
	...
	// Key
	стр_н = новый Структура;
	стр_н.Вставить("Свойство","Key");
	стр_н.Вставить("Синоним","Ключ");
	стр_н.Вставить("Значение","");
	стр_н.Вставить("ОписаниеТипов",ОписаниеТиповСтрока);
	стр_н.Вставить("ТолькоПросмотр",Ложь);
	стр_н.Вставить("Видимость",Ложь);
	...
	// Type
	стр_н = новый Структура;
	стр_н.Вставить("Свойство","Type");
	стр_н.Вставить("Синоним","Тип");
	стр_н.Вставить("Значение","");
	стр_н.Вставить("ОписаниеТипов",ОписаниеТиповСтрока);
	стр_н.Вставить("ТолькоПросмотр",Ложь);
	стр_н.Вставить("Видимость",Ложь);
...
Функция мСценРедактор_ПолучитьСписокСвойствДляДействияКоманды(Знач Действие="", Знач Команда="",знач API="")
	...
	ИначеЕсли Действие="НайтиОкно" Тогда
		...
		КонецЕсли;
		СоответсвиеДляПоиска.Получить("Интервал").Видимость = Истина;
	ИначеЕсли Действие="ПолучитьАктивноеОкно" Тогда
		СоответсвиеДляПоиска.Получить("ИмяПеременной").Видимость = Истина;
		СоответсвиеДляПоиска.Получить("ПродолжитьПриОшибке").Видимость = Истина;
		СоответсвиеДляПоиска.Получить("API").Видимость = Истина;
	ИначеЕсли Действие="НайтиФорму" Тогда
	...
	ИначеЕсли Действие="ПодключитьТестируемоеПриложение" или Действие="GenerateClientConnectionScript" Тогда
		...
		СоответсвиеДляПоиска.Получить("Интервал").Видимость = Истина;
		СоответсвиеДляПоиска.Получить("API").Видимость = Истина;
		СоответсвиеДляПоиска.Получить("TurnToCommentInBlock").Видимость = Истина;		
	...
	ИначеЕсли Действие="Таймер" Тогда
		СоответсвиеДляПоиска.Получить("Интервал").Видимость = Истина;
		СоответсвиеДляПоиска.Получить("API").Видимость = Истина;		
//	ИначеЕсли Действие="Логика" Тогда
//		СоответсвиеДляПоиска.Получить("API").Видимость = Истина;
//		СоответсвиеДляПоиска.Получить("Goal").Видимость = Истина;
	ИначеЕсли Действие="Замер" Тогда
		СоответсвиеДляПоиска.Получить("API").Видимость = Истина;
		СоответсвиеДляПоиска.Получить("ЗаголовокОбъекта").Видимость = Истина;
		СоответсвиеДляПоиска.Получить("Key").Видимость = Истина;
	...
	ИначеЕсли Действие="Команда" Тогда
		...
		Если (API="AutomationUI" ИЛИ API="Selenium") И (Найти(Команда,"attribute") ИЛИ Найти(Команда,"css") ИЛИ Найти(Команда,"get text") ИЛИ Найти(Команда,"get window title") 
			ИЛИ Найти(Команда,"get focused text") ИЛИ Найти(Команда,"is selected")) Тогда
			СоответсвиеДляПоиска.Получить("Attribute").Видимость = Истина;
			СоответсвиеДляПоиска.Получить("ИмяПараметра").Видимость = Истина;
			СоответсвиеДляПоиска.Получить("ЗначениеПараметра").Видимость = Истина;						
		КонецЕсли;
		
		Если API="Selenium" И (Найти(Команда,"get") ИЛИ  Найти(Команда,"is element stale"))Тогда
			СоответсвиеДляПоиска.Получить("ИмяПараметра").Видимость = Истина;
			//СоответсвиеДляПоиска.Получить("ЗначениеПараметра").Видимость = Истина;						
		КонецЕсли;
		...
		Если (API="AutomationUI" ИЛИ API="Selenium") И (Найти(Команда,"attribute") ИЛИ Найти(Команда,"get text") 
			ИЛИ Найти(Команда,"get window title") ИЛИ Найти(Команда,"get focused text")
			ИЛИ Найти(Команда,"get url")) Тогда
			СоответсвиеДляПоиска.Получить("Attribute").Видимость = Истина;
			СоответсвиеДляПоиска.Получить("ИмяПараметра").Видимость = Истина;
		...
		Если Команда="ВыполнитьКоманду" ИЛИ Команда="ExecuteCommand"  Тогда
		...
		Если Команда="ВыполнитьВыборИзВыпадающегоСписка" ИЛИ
			...
			Команда="НажатьНаГиперссылкуВФорматированнойСтроке" ИЛИ
			Команда="НажатьНаЭлементСостоянияПросмотра" ИЛИ
			Команда="УдалитьЭлементСостоянияПросмотра" 
			Тогда
			СоответсвиеДляПоиска.Получить("Presentation").Видимость = Истина;
		КонецЕсли;
		
		Если Команда="ПерейтиКСтроке" ИЛИ Команда="GotoRow" Тогда
			СоответсвиеДляПоиска.Получить("Direction").Видимость = Истина;
			СоответсвиеДляПоиска.Получить("RowDescription").Видимость = Истина;
		КонецЕсли;	
		
		Если Команда="ПерейтиНаУровеньВниз" ИЛИ Команда="GoOneLevelDown"
			ИЛИ Команда="ПерейтиНаУровеньВверх"  ИЛИ Команда="GoOneLevelUp" Тогда
			СоответсвиеДляПоиска.Получить("RowDescription").Видимость = Истина;
		КонецЕсли;	
		...
		Если Команда="УстановитьТекущуюОбласть" ИЛИ Команда="SetCurrentArea" Тогда
		...
		Если Команда="ПолучитьТекстыСообщенийПользователю" ИЛИ
			...
			Команда="ПолучитьТекстРедактирования" ИЛИ Команда="ПолучитьТекущуюИнформациюОбОшибке"
			ИЛИ  Команда="ПолучитьАдресТекущейОбласти" 
			ИЛИ  Команда="ПолучитьТекстОбласти"
			ИЛИ  Команда="ПолучитьТекстыЭлементовСостоянияПросмотра"
			Тогда
			СоответсвиеДляПоиска.Получить("ИмяПараметра").Видимость = Истина;
			СоответсвиеДляПоиска.Получить("ЗначениеПараметра").Видимость = Истина;
		КонецЕсли;
...
//
&НаКлиенте
Функция мСценРедактор_ПолучитьМассивСтруктурTypeПамять()
	
	СерьезностьМассив = новый Массив;
	
	// JSON Файл во временном каталоге
	стр_н = новый Структура;
	стр_н.Вставить("Type","TemporaryJSON");
	стр_н.Вставить("Синоним","Временный каталог + JSON");
	СерьезностьМассив.Добавить(стр_н);
	
	// JSON по пути/общий каталог
	стр_н = новый Структура;
	стр_н.Вставить("Type","CatalogJSON");
	стр_н.Вставить("Синоним","Каталог + JSON");
	СерьезностьМассив.Добавить(стр_н);
	
	// База данных и переменные задания
	стр_н = новый Структура;
	стр_н.Вставить("Type","1CBase");
	стр_н.Вставить("Синоним","База данных 1С");
	СерьезностьМассив.Добавить(стр_н);
	
	// Загрузить параметры подключения
	стр_н = новый Структура;
	стр_н.Вставить("Type","1CConnectionProperty");
	стр_н.Вставить("Синоним","Свойства подключения");
	СерьезностьМассив.Добавить(стр_н);
	
	// Загрузить параметры подключения
	стр_н = новый Структура;
	стр_н.Вставить("Type","1CScenarioParametersFromBase");
	стр_н.Вставить("Синоним","Параметры сценариев из базы");
	СерьезностьМассив.Добавить(стр_н);
...
Процедура мСценрРедактор_ПередатьИзмененияТаблицыСвойствВДеревоСценария()
	...
	ДействиеНовоеЗначение = Неопределено;
	КомандаСтароеЗначение = Неопределено;
	КомандаНовоеЗначение = Неопределено;
	ИзменяемоеСвойство = Неопределено;
	НовоеЗначение = Неопределено;
	
	ТекущиеДанные = Элементы.ТаблицаСвойства.ТекущиеДанные;
	Если НЕ ТекущиеДанные=Неопределено Тогда
		ИзменяемоеСвойство = ТекущиеДанные.Свойство;
		...
		Для каждого стр из ТаблицаСвойства Цикл
			Если стр.Свойство<>"ИдентификаторУзелаДерева" Тогда
				Если стр.Свойство="Действие" Тогда
					...
					ДействиеНовоеЗначение = стр.Значение;
				ИначеЕсли  стр.Свойство="Действие" Тогда
					КомандаСтароеЗначение = Узел[стр.Свойство];
					КомандаНовоеЗначение = стр.Значение;
				КонецЕсли;
	...
	Если Узел<>Неопределено Тогда	
		...
		КонецЕсли;
		// изменим картинку
		Если ДействиеСтароеЗначение<>Узел.Действие Тогда
			Узел.ДанныеКартинки = мСцен_ПолучитьДанныеКартинки_НаКлиенте(Узел);
		КонецЕсли;
	КонецЕсли;
	
	// Обработаем наименование
	Если Узел<>Неопределено Тогда	
		// если переключили на получение активного окна
		Если ДействиеНовоеЗначение="ПолучитьАктивноеОкно" И ДействиеНовоеЗначение<>ДействиеСтароеЗначение Тогда
			Узел.Наименование = "Получить активное окно";
		КонецЕсли;
		
		// добавим в наименование имя вызываемой формы
		Если ИзменяемоеСвойство="CommandRef" Тогда
			Если ЗначениеЗаполнено(НовоеЗначение) И СтрНайти(НовоеЗначение,"/") Тогда
				МассивЧастей = СтрРазделить(НовоеЗначение,"/?");
				Если МассивЧастей.Количество()>1 Тогда
					CommandRef_ИмяФормы = "";
					Если СтрНайти(МассивЧастей[МассивЧастей.ВГраница()],"ref=") Тогда
						CommandRef_ИмяФормы = МассивЧастей[МассивЧастей.ВГраница()-1];
					Иначе
						CommandRef_ИмяФормы = МассивЧастей[МассивЧастей.ВГраница()];
					КонецЕсли;
					Узел.Наименование = "Команда : ВыполнитьКоманду """+CommandRef_ИмяФормы+"""";
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура мСценРедактор_ИзменитьСписокВыбораПриИзмененииСтроки(Элемент,Знач Действие="",Знач ТипПоля = "")
	...
	ИначеЕсли Действие="Память" И ТипПоля="Команда" Тогда		
	
		ДоступныйСписокВыбора = мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандПамять();
		ИмяПоляЗначения = "Команда";
		
	ИначеЕсли Действие="Замер" И ТипПоля="Команда" Тогда		
	
		ДоступныйСписокВыбора = мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандЗамер();
		ИмяПоляЗначения = "Команда";
	...
	ИначеЕсли Действие="Память" И ТипПоля="Type" Тогда
		ДоступныйСписокВыбора = мСценРедактор_ПолучитьМассивСтруктурTypeПамять();
		ИмяПоляЗначения = "Type";
...
Процедура мСценРедактор_ЗаполнитьКнопкиПоУмолчанию()
	...
	СчетчикПеременных = новый Структура("НайтиОкно,НайтиОсновноеОкно,ПолучитьАктивноеОкно,НайтиФорму,НайтиОбъект,ПолучитьКомандныйИнтерфейс,Команда",1,1,1,1,1,1,1,1);
...
&НаКлиенте
Процедура мСценРедактор_ДеревоСценарияПриНачалеРедактированияФрагмент(Знач Элемент,Знач Копирование)
	...
	Иначе
		...
		мСценРедактор_ОпределитьНаименованиеПеременной(ТекущиеДанные);
	КонецЕсли;
		...
		Если ТекущиеДействие<>Неопределено Тогда
			...
			Если ТекущиеДанные.Действие = "ГотовыйБлокШагов" Тогда
				Если ТекущийГотовйШаг<>Неопределено Тогда
					...
					ТекущиеДанные.Наименование = "Библиотека """+ТекущийГотовйШаг.Наименование+"""";					
					ТекущиеДанные.UID = Строка(новый UUID());
					ТекущиеДанные.ЭтоБлок = Истина;
					
					// подгружаем файл блока шагов
					мОписаниеСценария = мСцен_ПолучитьСтруктуруОписанияСценария();
					Если НЕ Модуль_ВыгрузитьЗагрузитьФорматПроектаСценарияXML.ЗагрузитьСценарийФорматXML(мОписаниеСценария,ТекущиеДанные.ПутьБлокШагов,Истина) = Ложь Тогда
...
&НаКлиенте
Процедура мСценРедактор_ОпределитьНаименованиеПеременной(Знач ТекущиеДанные)
	
	Перем ИмяПеременнойРодителя, Родитель, ТипОбъектаРодителя;
...
Процедура мСценПакет_ПодготовкаЗапускАвтоТеста(ПутьКФайлу, ПутьКаталогаОтчета, ПаузаПослеЗаускаКлиентов=0)
	...
	мОписаниеСценария = мСцен_ПолучитьСтруктуруОписанияСценария();
	Модуль_ВыгрузитьЗагрузитьФорматПроектаСценарияXML.ЗагрузитьСценарийФорматXML(мОписаниеСценария,ПутьКФайлу);
	мСцен_ОбработатьСтруктуруОписанияСценария(мОписаниеСценария);
	...
	Если ИспользоватьКомандуЗапуститьСистему = Истина Тогда
		Для каждого стр из ПуллТестируемыхПриложений Цикл
			...
			ИначеЕсли ПараметрыПриложения.API = "Selenium" Тогда
				ТекстОшибки = Модуль_ЗаписьЖурналаДействий.ЗакрытьБраузер_Selenium_API(Объект.АдресИнтернетExternAutomationUI,Объект.НомерПортаExternAutomationUI, ПараметрыПриложения.НомерПорта);
...
Процедура ЗапуститьТестовыеПриложения(Знач СтрокаЗапуска,Знач НомерПорта=0,Знач API="1cClientApp",Знач Браузер="Chrome") Экспорт
	...
	Если API="Selenium" Тогда // Запускаем браузеры
		ТекстОшибки = Модуль_ЗаписьЖурналаДействий.ЗапуститьБраузер_Selenium_API(Браузер,Объект.АдресИнтернетExternAutomationUI,Объект.НомерПортаExternAutomationUI,НомерПорта,Объект.ПутьИсполняемыйФайлБраузера);
...
Процедура мСценРедактор_ИзменитьПодсказкуTips(ТекущиеДанные)
	...
	ИначеЕсли ТекущиеДанные.Действие="ЗначениеВПараметр" Тогда
		ТекстПодсказки = "&nbsp;Копирует значение введнное в свойство представление в значение переменной с именем 'ИмяПараметра'.";
	...
	ИначеЕсли ТекущиеДанные.Действие="Команда" Тогда
		...
		Если Объект.ИспользуемыйAPI="Selenium" Тогда
			...
			ИначеЕсли НРЕГ(ТекущиеДанные.Команда)="get url" Тогда
				ТекстПодсказки = ТекстПодсказки+"<br /> <b>get url</b> - получает текущий адрес браузера и сохраняет его в параметр";
...
Процедура СохранитьИзменениеВТаблицуИзменений(МассивИзменений,Наименование="")
	...
	// не сохраняем изменения, когда тест запущен
	Если Объект.ИспользоватьСохранениеДерева=Ложь И ТестЗапущен=Ложь Тогда
		Возврат;
	КонецЕсли;
...
#Область ТекстовыйКодСценария

&НаКлиенте
Функция ПолучитьКонструкторHTML(ИмяЭлементаHTML)
	
	Перем htmlView;
	
	htmlView = неопределено;
	Если Элементы[ИмяЭлементаHTML].Документ.parentWindow=Неопределено Тогда
		htmlView = Элементы[ИмяЭлементаHTML].Документ.defaultView;
	Иначе
		htmlView = Элементы[ИмяЭлементаHTML].Документ.parentWindow;
	КонецЕсли;
	Возврат htmlView;
...
&НаКлиенте
Функция ПреобразоватьДеревоСценарияВТекст(ПреобразовыватьБлокиШагов=Ложь) Экспорт
	Возврат ПреобразоватьДеревоТекст(ДеревоСценария,"",ПреобразовыватьБлокиШагов,Ложь);
КонецФункции

&НаКлиенте
Функция ПреобразоватьДеревоТекст(УзелРодитель,Отступ="",ПреобразовыватьБлокиШагов=Ложь,РежимHTML=Истина) Экспорт
	
	Текст = "";
	
	
	Для каждого Узел из УзелРодитель.ПолучитьЭлементы() Цикл
		Если РежимHTML=Истина Тогда
			Текст = Текст+"<li id='"+Узел.UID+"'>"+Узел.Наименование;
			Если Узел.ЭтоБлок=Истина И Узел.Действие="ГотовыйБлокШагов" Тогда 
				Если ПреобразовыватьБлокиШагов=Истина Тогда
					Текст = Текст+ПреобразоватьДеревоТекст(Узел,Отступ,ПреобразовыватьБлокиШагов,РежимHTML);
				КонецЕсли;
			Иначе
				Текст = Текст+ПреобразоватьДеревоТекст(Узел,Отступ,ПреобразовыватьБлокиШагов,РежимHTML);
			КонецЕсли;
			Текст = Текст+"</li>";
		Иначе
			Текст = Текст+Отступ+Узел.Наименование+"  "+Символы.ПС;;
			Если Узел.ЭтоБлок=Истина И Узел.Действие="ГотовыйБлокШагов" Тогда 
				Если ПреобразовыватьБлокиШагов=Истина Тогда
					Текст = Текст+ПреобразоватьДеревоТекст(Узел,Отступ+" ",ПреобразовыватьБлокиШагов,РежимHTML);
				КонецЕсли;
	...
	Если УзелРодитель.ПолучитьЭлементы().Количество()>0 Тогда
		Если РежимHTML=Истина Тогда
			Текст = "<ol>"+Текст+"</ol>";
		Иначе
...
КонецФункции
&НаКлиенте
Процедура мСценСкрипт_ПреобразоватьДеревоТекст(Команда)
	Текст = ПреобразоватьДеревоТекст(ДеревоСценария,"",Истина,Истина);
	htmlView = ПолучитьКонструкторHTML("КонструкторHTML");
	htmlView.set_text(Текст);
КонецПроцедуры

&НаКлиенте
Процедура мСценСкрипт_ПреобразоватьТекстДерево(Команда)
	ПреобразоватьТекстДерево(ДеревоСценария);
КонецПроцедуры

&НаКлиенте
Процедура ПреобразоватьТекстДерево(Узел)
КонецПроцедуры

&НаКлиенте
Процедура КонструкторHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	
	НажатыйЭлемент = ДанныеСобытия.Element;
	Если НажатыйЭлемент.id = "interactionButton" Тогда	
		htmlView = ПолучитьКонструкторHTML("КонструкторHTML");
		//Сообщить(htmlView.interactionVariable);
		//ОбработатьКоманду(htmlView.interactionVariable,htmlView.interactionVariableAdditional);
		мСценСкрипт_РазвернутьДеревоВТаблицу(Истина,Истина);
		мОтбор = новый Структура("UID",htmlView.interactionVariable);
		н_строки = ТаблицаСценария.НайтиСтроки(мОтбор);
		Если н_строки.Количество()=0 Тогда
			Возврат;
		КонецЕсли;
		Элементы.ДеревоСценария.ТекущаяСтрока = н_строки[0].ИдентификаторУзелаДерева;
...
// Функция - Получить кеш структур выходных параметров сценария
// 
// Возвращаемое значение:
//  соответствие - список параметров для обработки модулем логики
//
&НаКлиенте
Функция ПолучитьКешСтруктурВыходныхПараметровСценария() Экспорт
	
	КешВыходных = новый Соответствие;
	
	Для каждого стр из Логика_Effects Цикл
		КешВыходных.Вставить(стр.Variable,стр);
	КонецЦикла;
	
	КешСтруктур = новый Соответствие;
	
	Для каждого стр из ПараметрыСценария Цикл
		Если НЕ ЗначениеЗаполнено(стр.СвязьЛогика) 
			ИЛИ КешВыходных.Получить(стр.СвязьЛогика)=Неопределено Тогда
			Продолжить;
		КонецЕсли;
		СтруктураПараметра = новый Структура;
		СтруктураПараметра.Вставить("СвязьЛогика",стр.СвязьЛогика);
		СтруктураПараметра.Вставить("Имя",стр.Имя);
		СтруктураПараметра.Вставить("ТипДанных",стр.ТипДанных);
		СтруктураПараметра.Вставить("Значение",стр.Значение);
		КешСтруктур.Вставить(стр.СвязьЛогика,СтруктураПараметра);
	КонецЦикла;
...
&НаКлиенте
Функция ПолучитьПараметрыРаботыСкрипта() Экспорт
	Если НЕ ЗначениеЗаполнено(мПараметрыРаботыСкрипта) Тогда
		мПараметрыРаботыСкрипта = новый Структура("ТестовоеПриложение,мПараметрыКлиент,мПараметрыКлиентСервер",Неопределено,новый Структура(),новый Структура());
	КонецЕсли;
	Возврат мПараметрыРаботыСкрипта;
КонецФункции


// Процедура - Установить кеш структур входных параметров сценария
//
// Параметры:
//  КешСтруктур	 - соответствие	 - список свойств для обновления параметров
//
&НаКлиенте
Процедура УстановитьКешСтруктурВходныхПараметровСценария(КешСтруктур) Экспорт
	
	КешВходных = новый Соответствие;
	
	Для каждого стр из Логика_Preconditions Цикл
		КешВходных.Вставить(стр.Variable,стр);
	КонецЦикла;
	
	
	Для каждого стр из ПараметрыСценария Цикл
		Если НЕ ЗначениеЗаполнено(стр.СвязьЛогика) 
			ИЛИ КешВходных.Получить(стр.СвязьЛогика)=Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураПараметра = КешСтруктур.Получить(нрег(стр.СвязьЛогика));
		
		Если СтруктураПараметра=Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		стр.Значение = СтруктураПараметра.Значение;
		СтруктураПараметра.Изменено = Истина;
...
&НаКлиенте
Процедура УстановитьПараметрыМенеджера(мПараметры) Экспорт
	ЗаполнитьЗначенияСвойств(Объект,мПараметры);
КонецПроцедуры


&НаКлиенте
Процедура УстановитьПараметрыРаботыСкрипта(мПараметры) Экспорт
	мПараметрыРаботыСкрипта = мПараметры;
КонецПроцедуры


&НаКлиенте
Процедура мСценСкрипт_ПоискПодобияШаблона(Команда)
	мПараметры = Новый Структура("ПутьККаталогуБиблиотекиСценариев,ТекстСценария",Объект.ПутьККаталогуБиблиотекиСценариев,ПреобразоватьДеревоСценарияВТекст());
	Форма = ПолучитьФорму("ВнешняяОбработка.МенеджерСценарногоТеста.Форма.Форма_АнализаСценария",мПараметры,ЭтаФорма);
	Форма.Открыть();
КонецПроцедуры  


&НаКлиенте
Процедура ПутьКаталогБраузераНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие); 
	Диалог.Заголовок = "Выберите исполняемый файлом браузера"; 
	Диалог.ПолноеИмяФайла = ""; 
	Фильтр = "Exe-файл (*.exe)|*.exe"; 
	Фильтр = ""; 
	Диалог.Фильтр = Фильтр; 
	Диалог.МножественныйВыбор = Ложь; 
	Если ЗначениеЗаполнено(Объект.ПутьИсполняемыйФайлБраузера) Тогда
		Диалог.Каталог = ПолучитьКаталогПоПутиФайла(Объект.ПутьИсполняемыйФайлБраузера);
	КонецЕсли;	
	Если Диалог.Выбрать() Тогда 
		Объект.ПутьИсполняемыйФайлБраузера = Диалог.ПолноеИмяФайла; 
	КонецЕсли; 
КонецПроцедуры

&НаКлиенте
Процедура РазделыСправкиПриИзменении(Элемент)
	AboutHTML = ПолучитьHMLТекстМакета(РазделыСправки);
КонецПроцедуры
