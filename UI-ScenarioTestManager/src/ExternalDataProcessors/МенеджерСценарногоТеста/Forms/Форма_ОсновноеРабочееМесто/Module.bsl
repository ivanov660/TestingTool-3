...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ТипТестируемогоКлиента) Тогда
		Объект.ТипТестируемогоКлиента = "1cClientApp";	
	КонецЕсли;
	Элементы.ТипТестируемогоКлиента.СписокВыбора.Добавить("1cClientApp","1C клиент");
	Элементы.ТипТестируемогоКлиента.СписокВыбора.Добавить("WebBrowser","Веб браузер");
	Элементы.ТипТестируемогоКлиента.СписокВыбора.Добавить("WinDesktopApp","Настольное приложение");
...
Процедура мСцен_ПреобразоватьВДеревоСценарияНаКлиенте(ТекстСценария,Модуль_ПреобразоватьЖурналДействийВДеревоСценария,ИмяПриемника)
	...
	мПараметры = Новый Структура("ГенерироватьПолучениеРодительскихОбъектовВИерархии",Объект.ГенерироватьПолучениеРодительскихОбъектовВИерархии);
...
Процедура мСценСкрипт_СделатьШагПоТаблицеСценария()
	...
	Если ИндексТекущегоШага<ТаблицаСценария.Количество() И РучнойРежим=Ложь Тогда
	...
	Объект.ВремяВыполнения = ВремяОкончанияаТеста-ВремяНачалаТеста;
...
Процедура мСценСкрипт_ОбработатьТипУзла(Узел,ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,мПараметрыКлиентСервер)
	...
	Если Узел.Действие="ПодключитьТестируемоеПриложение" ИЛИ Узел.Действие="GenerateClientConnectionScript" Тогда
		...
		мСценСкрипт_GenerateClientConnection(ТестовоеПриложение,ОписаниеОшибки,Узел.СвойстваПодключенияКлиентаТестирования,Узел.Интервал);
		
	ИначеЕсли Узел.Действие="GenerateClientDisconnectionScript" ИЛИ Узел.Действие="ЗакрытьТестируемоеПриложение" Тогда
		
		мСценСкрипт_GenerateClientDisconnection(ТестовоеПриложение,ОписаниеОшибки,Узел.СвойстваПодключенияКлиентаТестирования);
	...
	ИначеЕсли Узел.Действие = "ПолучитьПредставлениеДанных" Тогда
		...
		мСценСкрипт_ПолучитьПредставлениеДанных(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,мПараметрыКлиент[Узел.ИмяПеременной],Узел.ПредставлениеДанных,Узел.ИмяПараметра,Узел.ЗначениеПараметра);
		
	ИначеЕсли Узел.Действие = "СравнитьСПредставлениемДанных" Тогда
		
		мСценСкрипт_СравнитьСПредставлениемДанных(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,мПараметрыКлиент[Узел.ИмяПеременной],Узел.ПредставлениеДанных,Узел.УсловиеСравнения);
	...
	ИначеЕсли Узел.Действие="ДилогВыбораФайла" ИЛИ Узел.Действие="FileDialog" Тогда
		...
		мСценСкрипт_setFileDialogResult(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиентСервер,Узел.filename);
	...
	ИначеЕсли Узел.Действие="НайтиОсновноеОкно" Тогда
		...
		мСценСкрипт_НайтиОсновноеОкно(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент[Узел.ИмяПеременной]);
		
	ИначеЕсли Узел.Действие="НайтиОкно" ИЛИ Узел.Действие="FindWindow" Тогда
		
		мСценСкрипт_НайтиОкно(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент[Узел.ИмяПеременной],ЗаголовокОбъекта,Узел.ИспользоватьВариантыПоиска);
		
	ИначеЕсли Узел.Действие="НайтиФорму" ИЛИ Узел.Действие="FindForm" Тогда	
		
		мСценСкрипт_НайтиФорму(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент[Узел.ИмяПеременной],мПараметрыКлиент[Узел.ПолучитьРодителя().ИмяПеременной],ЗаголовокОбъекта,Узел.ИспользоватьВариантыПоиска);
		
	ИначеЕсли Узел.Действие = "ПолучитьКомандныйИнтерфейс" ИЛИ Узел.Действие = "GetCommandInterface" Тогда
		
		мСценСкрипт_ПолучитьКомандныйИнтерфейс(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент[Узел.ИмяПеременной],мПараметрыКлиент[Узел.ПолучитьРодителя().ИмяПеременной]);
		
	ИначеЕсли Узел.Действие = "НайтиОбъект" ИЛИ Узел.Действие = "FindObject" Тогда
		
		мСценСкрипт_НайтиОбъект(ТестовоеПриложение,ОписаниеОшибки,Узел.ТипОбъекта,мПараметрыКлиент[Узел.ИмяПеременной],мПараметрыКлиент[Узел.ПолучитьРодителя().ИмяПеременной],ЗаголовокОбъекта,Узел.ИмяОбъекта,Узел.ИспользоватьВариантыПоиска);
		
	ИначеЕсли Узел.Действие = "ПроверкаНаличияЭлемента" Тогда
	
		мСценСкрипт_ПроверкаНаличияЭлемента(ТестовоеПриложение,ОписаниеОшибки,Узел.ТипОбъекта,ЗаголовокОбъекта,Узел.ИмяОбъекта,Узел.ИмяПараметра,Узел.ИспользоватьВариантыПоиска);
	...
	ИначеЕсли Узел.Действие = "Команда" ИЛИ Узел.Действие = "Command" Тогда
		...
		//Если ИмяПеременнойРодителя="" И Тогда
		//	ВызватьИсключение "Для действия ["+Узел.Наименование+"] не указана переменная!
		//	| К текущему родителю ["+Узел.ПолучитьРодителя().Наименование+"] не применимо! Уажите имя переменной!";
		//КонецЕсли;
		
		// если имя переменной не задано, тогда присвоим ей родителя, т.к. все кнопки и действия принадлежат обычно родителю
		Если мПараметрыКлиент[Узел.ИмяПеременной]=Неопределено Тогда
			мПараметрыКлиент[Узел.ИмяПеременной] = РодительПеременная;
		КонецЕсли;
		
		мСценСкрипт_ВыполнитьКоманду(ТестовоеПриложение,ОписаниеОшибки,Узел.Команда,Узел.ТипОбъекта,мПараметрыКлиент[Узел.ИмяПеременной],РодительПеременная,ЗаголовокОбъекта,OutputText,Узел.CommandRef,Presentation,Узел.Direction,Узел.RowDescription,Узел.SwitchSelection,Узел.Cancel,Узел.Area,Узел.ИмяПараметра,Узел.ЗначениеПараметра);
		
	ИначеЕсли Узел.Действие = "Условие" ИЛИ Узел.Действие = "Сondition" Тогда
		
		мСценСкрипт_ОбработатьУсловие(ТестовоеПриложение,ОписаниеОшибки, Узел, Узел.ИмяПараметра,Узел.ЗначениеПараметра,Узел.УсловиеСравнения);
	...
	Если ТестовоеПриложение<>Неопределено Тогда
		...
		Если ИнформацияОбОшибке<>Неопределено Тогда
			...
			ОписаниеОшибки = ОписаниеОшибки+Символы.ПС+НСтр("ru='Описание=';en='Description='") + ИнформацияОбОшибке.Описание + "'";
			ОписаниеОшибки = ОписаниеОшибки+Символы.ПС+НСтр("ru='ИмяМодуля=';en='ModuleName='") + ИнформацияОбОшибке.ИмяМодуля + "'";
			ОписаниеОшибки = ОписаниеОшибки+Символы.ПС+НСтр("ru='НомерСтроки=';en='LineNumber='") + ИнформацияОбОшибке.НомерСтроки + "'";
			ОписаниеОшибки = ОписаниеОшибки+Символы.ПС+НСтр("ru='ИсходнаяСтрока=';en='SourceLine='") + ИнформацияОбОшибке.ИсходнаяСтрока + "'";
...
&НаКлиенте
Процедура мСценСкрипт_GenerateClientConnection(ТестовоеПриложение,ОписаниеОшибки,Знач СвойстваПодключенияКлиентаТестирования,Знач Интервал=0)	
	
	ТестовоеПриложение = Неопределено;
	НомерПорта = Объект.НомерПорта; // порт по умолчанию
	...
	Если Найти(СвойстваПодключенияКлиентаТестирования,"&") Тогда
		...
		СвойстваПодключенияКлиентаТестирования = мСцен_ПолучитьЗначениеПараметра(ИмяПараметра);
	КонецЕсли;	
...
&НаКлиенте
Процедура мСценСкрипт_GenerateClientDisconnection(ТестовоеПриложение,ОписаниеОшибки,СвойстваПодключенияКлиентаТестирования)	
...
&НаКлиенте
Процедура мСценСкрипт_ПолучитьПредставлениеДанных(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,ТекущаяПеременная,ПредставлениеДанных,ИмяПараметра,ЗначениеПараметра)
	...
	Попытка
		...
		Иначе
			...
			мСцен_УстановитьЗначениеПараметра(ИмяПараметра,ЗначениеПараметра);
		КонецЕсли; 	
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
		ВызватьИсключение ОписаниеОшибки;
...
&НаКлиенте
Процедура мСценСкрипт_СравнитьСПредставлениемДанных(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,ТекущаяПеременная,ПредставлениеДанных,УсловиеСравнения="")
	...
	Попытка
		...
		Иначе
			...
			ПараметрПредставлениеДанных = мСцен_ПолучитьЗначениеПараметра(ИмяПараметра);
			...
			ОписаниеОшибки = мСценСкрипт_СравнитьСПредставлениемДанныхФрагмент(УсловиеСравнения, ЗначениеПредставлениеДанных, ТекущееПредставлениеДанных);
...
&НаКлиенте
Функция мСценСкрипт_СравнитьСПредставлениемДанныхФрагмент(Знач УсловиеСравнения, ШаблонПроверки, ПроверяемыйТекст)
	...
	Если Врег_УсловиеСравенения="РАВНО" Тогда
		Если ШаблонПроверки <> ПроверяемыйТекст Тогда
			ОписаниеОшибки = "ПредставлениеДанных:("+ШаблонПроверки+") <> ПолучитьПредставлениеДанных: ("+ПроверяемыйТекст+")";
		КонецЕсли;
	...
	ИначеЕсли Врег_УсловиеСравенения="НЕЗАПОЛНЕНО" Тогда
		Если ЗначениеЗаполнено(ПроверяемыйТекст) Тогда
			ОписаниеОшибки = "Значение ЗАПОЛНЕНО ПолучитьПредставлениеДанных: ("+ПроверяемыйТекст+"), а не должно быть заполнено";
		КонецЕсли;
	ИначеЕсли Врег_УсловиеСравенения="СОДЕРЖИТ" Тогда
		Если НЕ Найти(ПроверяемыйТекст,ШаблонПроверки) Тогда
			ОписаниеОшибки = "ПолучитьПредставлениеДанных: ("+ПроверяемыйТекст+") не содержит ПредставлениеДанных:("+ШаблонПроверки+"), а должно ";
		КонецЕсли;
	ИначеЕсли Врег_УсловиеСравенения="НЕСОДЕРЖИТ" Тогда
		Если Найти(ПроверяемыйТекст,ШаблонПроверки) Тогда
			ОписаниеОшибки = "ПолучитьПредставлениеДанных: ("+ПроверяемыйТекст+") содержит ПредставлениеДанных:("+ШаблонПроверки+"), а не должно! ";
		КонецЕсли;
...
&НаКлиенте
Процедура мСценСкрипт_ОбработатьУсловие(ТестовоеПриложение,ОписаниеОшибки, Узел, ИмяПараметра,ЗначениеПараметра,УсловиеСравнения)
	
	Попытка
		ЗначениеПараметраТаблицыПараметров = мСцен_ПолучитьЗначениеПараметра(ИмяПараметра);
		
		РезультатСравенения = мСценСкрипт_СравнитьСПредставлениемДанныхФрагмент(УсловиеСравнения,ЗначениеПараметра,ЗначениеПараметраТаблицыПараметров);
...
&НаКлиенте
Процедура мСценСкрипт_setFileDialogResult(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиентСервер,filename)
...
&НаКлиенте
Процедура мСценСкрипт_НайтиОсновноеОкно(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная)
...
&НаКлиенте
Процедура мСценСкрипт_НайтиОкно(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная,Знач ЗаголовокОбъекта,ИспользоватьВариантыПоиска=Ложь)
	...
	Если ИспользоватьВариантыПоиска=Истина Тогда
		...
		Для ш=0 по МассивЗаголовковОбъектов.Количество()-1 Цикл
			...
			ТекущаяПеременная = ТестовоеПриложение.НайтиОбъект(Тип("ТестируемоеОкноКлиентскогоПриложения"), DoubleQuotationMarks(ЗаголовокОбъекта) , , 5);
...
&НаКлиенте
Процедура мСценСкрипт_НайтиФорму(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная,РодительПеременная,Знач ЗаголовокОбъекта,ИспользоватьВариантыПоиска=Ложь)
	...
	Если ИспользоватьВариантыПоиска=Истина Тогда
		...
		Для ш=0 по МассивЗаголовковОбъектов.Количество()-1 Цикл
			...
			ТекущаяПеременная = РодительПеременная.НайтиОбъект(Тип("ТестируемаяФорма"), DoubleQuotationMarks(ЗаголовокОбъекта));
...
&НаКлиенте
Процедура мСценСкрипт_ПолучитьКомандныйИнтерфейс(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная,РодительПеременная)
...
&НаКлиенте
Процедура мСценСкрипт_НайтиОбъект(ТестовоеПриложение,ОписаниеОшибки,Знач ТипОбъекта,ТекущаяПеременная,РодительПеременная,Знач ЗаголовокОбъекта,Знач ИмяОбъекта,ИспользоватьВариантыПоиска=Ложь)
	...
	Если ИспользоватьВариантыПоиска=Истина Тогда
		...
		Для ш=0 по Макс(МассивИменОбъектов.Количество(),МассивЗаголовковОбъектов.Количество())-1 Цикл
			...
			Если ЗначениеЗаполнено(Тип) Тогда
				ТекущаяПеременная = РодительПеременная.НайтиОбъект(Тип(Тип), ?(ЗаголовокОбъекта<>"",DoubleQuotationMarks(ЗаголовокОбъекта),Неопределено),?(ИмяОбъекта<>"",DoubleQuotationMarks(ИмяОбъекта),Неопределено));
			Иначе
				ТекущаяПеременная = РодительПеременная.НайтиОбъект(,DoubleQuotationMarks(ЗаголовокОбъекта));
			КонецЕсли;
...
&НаКлиенте
Процедура мСценСкрипт_ПроверкаНаличияЭлемента(ТестовоеПриложение, ОписаниеОшибки, Знач ТипОбъекта, Знач ЗаголовокОбъекта,Знач ИмяОбъекта,Знач ИмяПараметра, ИспользоватьВариантыПоиска=Ложь)
	...
	Если ТипОбъекта="Form" или ТипОбъекта="ClientApplicationWindow" Тогда
		...
		Если ИспользоватьВариантыПоиска=Истина Тогда
			...
			Для ш=0 по МассивЗаголовковОбъектов.Количество()-1 Цикл
				...
				ТекущаяПеременная = ТестовоеПриложение.НайтиОбъект(Тип("ТестируемаяФорма"), DoubleQuotationMarks(ЗаголовокОбъекта));
	...
	Иначе
		...
		Если ИспользоватьВариантыПоиска=Истина Тогда
			...
			Для ш=0 по Макс(МассивИменОбъектов.Количество(),МассивЗаголовковОбъектов.Количество())-1 Цикл
				...
				Если ЗначениеЗаполнено(Тип) Тогда
					ТекущаяПеременная = ТестовоеПриложение.НайтиОбъект(Тип(Тип), ?(ЗаголовокОбъекта<>"",DoubleQuotationMarks(ЗаголовокОбъекта),Неопределено),?(ИмяОбъекта<>"",DoubleQuotationMarks(ИмяОбъекта),Неопределено));
				Иначе
					ТекущаяПеременная = ТестовоеПриложение.НайтиОбъект(,DoubleQuotationMarks(ЗаголовокОбъекта));
				КонецЕсли;
	...
	Значение = мСцен_ПолучитьЗначениеПараметра(ИмяПараметра);
	Если ТипЗнч(Значение)=Тип("Строка") Тогда
		мСцен_УстановитьЗначениеПараметра(ИмяПараметра,Строка(ОбъектНайден));
	ИначеЕсли ТипЗнч(Значение)=Тип("Число") Тогда
		мСцен_УстановитьЗначениеПараметра(ИмяПараметра,?(ОбъектНайден,1,0));	
	Иначе
		мСцен_УстановитьЗначениеПараметра(ИмяПараметра,ОбъектНайден);	
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура мСценСкрипт_ПерейтиКСтроке(ТестовоеПриложение,ОписаниеОшибки,ТипОбъекта,ТекущаяПеременная,РодительПеременная,ЗаголовокОбъекта,Direction,RowDescription,SwitchSelection)
	...
	Если ТипЗнч(ОписаниеСтроки)=Тип("Соответствие") Тогда
		Для каждого стр из ОписаниеСтроки Цикл
			...
			мСценСкрипт_ПодставитьЗначениеПараметра(Значение);
...
&НаКлиенте
Процедура мСценСкрипт_ВыполнитьКоманду(ТестовоеПриложение,ОписаниеОшибки,ИмяКоманды,ТипОбъекта,ТекущаяПеременная,РодительПеременная,ЗаголовокОбъекта,OutputText,CommandRef,Presentation,Direction,RowDescription,SwitchSelection,Cancel,Area,ИмяПараметра,ЗначениеПараметра)
	...
	ИначеЕсли ИмяКоманды = "ПерейтиКСтроке" ИЛИ ИмяКоманды = "GotoRow" Тогда 
		мСценСкрипт_ПерейтиКСтроке(ТестовоеПриложение,ОписаниеОшибки,ТипОбъекта,ТекущаяПеременная,РодительПеременная,ЗаголовокОбъекта,Direction,RowDescription,SwitchSelection);
	...
	ИначеЕсли ИмяКоманды = "ВвестиТекст" Тогда
		ТекущаяПеременная.ВвестиТекст("" + DoubleQuotationMarks(OutputText) + "");	
	ИначеЕсли ИмяКоманды = "ВыполнитьКоманду" Тогда
		ТекущаяПеременная.ВыполнитьКоманду("" + DoubleQuotationMarks(CommandRef) + "");
	ИначеЕсли ИмяКоманды = "ВыполнитьВыборИзВыпадающегоСписка" ИЛИ ИмяКоманды = "ExecuteChoiceFromDropList" Тогда
		...
		ТекущаяПеременная.ВыполнитьВыборИзВыпадающегоСписка("" + DoubleQuotationMarks(Presentation) + "");
	ИначеЕсли ИмяКоманды = "ВыполнитьВыборИзМеню" ИЛИ ИмяКоманды = "ExecuteChoiceFromMenu" Тогда
		ТекущаяПеременная.ВыполнитьВыборИзМеню("" + DoubleQuotationMarks(Presentation) + "");
	ИначеЕсли ИмяКоманды = "ВыполнитьВыборИзСпискаВыбора" ИЛИ ИмяКоманды = "ExecuteChoiceFromChoiceList" Тогда
		ТекущаяПеременная.ОжидатьФормированияВыпадающегоСписка();
		ТекущаяПеременная.ВыполнитьВыборИзСпискаВыбора("" + DoubleQuotationMarks(Presentation) + "");
	ИначеЕсли ИмяКоманды = "ВыбратьВариант" ИЛИ ИмяКоманды = "SelectOption" Тогда 
		// Виссаров ES- [11.10.2016] ++
		ТекущаяПеременная.ВыбратьВариант("" + DoubleQuotationMarks(Presentation) + "");
		// Виссаров ES- [11.10.2016] --
	...
	Если ЗапуститьКлиентаПоУмолчанию=Ложь Тогда
		...
		Для каждого стр из СписокКлиентовТестирования Цикл
			...
			Попытка
				...
				Если ТипЗнч(ОписаниеПодключения)=Тип("Соответствие") Тогда
					Если ОписаниеПодключения.Получить("НастройкиПоУмолчанию")=Ложь Тогда
						НомерПорта = ОписаниеПодключения.Получить("НомерПорта");
						...
						+ ОписаниеПодключения.Получить("СтрокаПодключения")+""" /TESTCLIENT" 
						+ " -TPort "+СтрЗаменить(Строка(НомерПорта),Символы.НПП,"")+" "
						+ ?(ЗначениеЗаполнено(ОписаниеПодключения.Получить("Пользователь1С"))," /N """+ОписаниеПодключения.Получить("Пользователь1С")+""" "," ")
						+ ?(ЗначениеЗаполнено(ОписаниеПодключения.Получить("Пользователь1С")) И ЗначениеЗаполнено(ОписаниеПодключения.Получить("Пароль1С")),"/P """+ОписаниеПодключения.Получить("Пароль1С")+""" ","")+" "
			...
			Если КлиентТестированияПоУмолчанию=Истина Тогда
				ЗапуститьТестовыеПриложения(ПутьЗапускаПоУмолчанию,НомерПорта);
			Иначе
				ЗапуститьТестовыеПриложения(ПутьЗапуска,НомерПорта);
			КонецЕсли;
			
		КонецЦикла; 
		
	Иначе 
		ЗапуститьТестовыеПриложения(ПутьЗапускаПоУмолчанию,Объект.НомерПорта);
	КонецЕсли;
...
//
&НаКлиенте
Процедура мСценСкрипт_СбросПараметровПередЗапускомСценария()
	ИндексТекущегоШага = 0;
	ИндексПереходСТекущегоШага = 0;
...
Функция мСценСкрипт_ПолучитьДетальныйРезультатВыполненияСценария(ВыгружатьШаги=Ложь)
	...
	ТестРезультатСтруктура.Вставить("ИдентификаторТеста",Объект.ИдентификаторТеста);
	ТестРезультатСтруктура.Вставить("ВремяВыполнения",Окр(Объект.ВремяВыполненияUTC/1000,2,РежимОкругления.Окр15как20));
	...
	ТестРезультатСтруктура.Вставить("time",Окр((ВремяОкончанияаТестаUTC-ВремяНачалаТестаUTC)/1000,2,РежимОкругления.Окр15как20)); 
	...
	// 1.2 Просуммируем время
	Для ш=0 по ТаблицаСценария.Количество()-1 Цикл
		стр = ТаблицаСценария[ш];
		Узел = ДеревоСценария.НайтиПоИдентификатору(стр.ИдентификаторУзелаДерева);
		УзелРодитель = Узел.ПолучитьРодителя();
		
		Если ш+1=ТаблицаСценария.Количество() Тогда
		Иначе
			ДатаНачалаВыполненияСледующегоШагаUTC = ДеревоСценария.НайтиПоИдентификатору(ТаблицаСценария[ш+1].ИдентификаторУзелаДерева).UTC;
			Если НЕ ДатаНачалаВыполненияСледующегоШагаUTC = 0 Тогда
				ТестРезультатСтруктура.time = ТестРезультатСтруктура.time + Окр((ДатаНачалаВыполненияСледующегоШагаUTC-Узел.UTC)/1000,2,РежимОкругления.Окр15как20);
			КонецЕсли;				
		КонецЕсли;
	...
	Для каждого стр из ТаблицаСценария Цикл
		...
		СтруктураШага = новый Структура("Шаги,name,title,start,stop,РезультатВыполнения,ТекстОшибки,НомерШага,Действие,Команда,severity,status",новый Массив,
			Узел.Наименование,Узел.Наименование,Узел.UTC,Узел.UTC,Узел.РезультатВыполнения,Узел.ТекстОшибки,Узел.НомерШага,Узел.Действие,Узел.Команда,Узел.Severity,Неопределено);
	...
	Для каждого стр из ТаблицаСценария Цикл
		...
		Если СтруктураШага=Неопределено ИЛИ СтруктураШага=Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	// 2 все остальные тестовые случаи искусственные, для выделения блоков
	Для каждого стр из ТаблицаСценария Цикл
		Узел = ДеревоСценария.НайтиПоИдентификатору(стр.ИдентификаторУзелаДерева);
		Если Узел.Действие="ТестовыйСлучай" Тогда 			
			
			// описание тестового случая
			мСтруктура = Новый Структура();
			мСтруктура.Вставить("Имя",Узел.Наименование);
			мСтруктура.Вставить("ВремяВыполнения",0);
			мСтруктура.Вставить("РезультатВыполнения",0); 
			мСтруктура.Вставить("time",0); 		
			мСтруктура.Вставить("start",Узел.UTC); 
			мСтруктура.Вставить("stop",0); 
			мСтруктура.Вставить("ТестПройден",Истина);
			мСтруктура.Вставить("ТекстОшибки",""); 
			мСтруктура.Вставить("КоличествоШаговОшибок",0);
			мСтруктура.Вставить("Severity",?(ЗначениеЗаполнено(Узел.Severity),Узел.Severity,"Значительная")); 
			мСтруктура.Вставить("Шаги",новый Массив);
			
			мСценСкрипт_ПолучитьРезультатВыполнениеПоУзлуВниз(Узел,мСтруктура);
			
			мСтруктура.time = мСтруктура.stop-мСтруктура.start;
			
			Если мСтруктура.КоличествоШаговОшибок<>0 Тогда
				мСтруктура.ТестПройден = Ложь;
			КонецЕсли;
			//мСценСкрипт_ПолучитьКодОшибкиПоРезультатуВыполнения(мСтруктура, Узел);
			
			// 3 добавляем шаги
			//мСценСкрипт_ВставитьШаги
			
			ТестРезультатСтруктура.ТестовыеСлучаи.Добавить(мСтруктура);
		КонецЕсли;	
	...
	// если тестовый случай один и корневой, тогда нет смысла добавлять новые
	Если ТестРезультатСтруктура.ТестовыеСлучаи.Количество()=1 и ТаблицаСценария.Количество()>0 Тогда
		стр = ТаблицаСценария[0];
		Узел = ДеревоСценария.НайтиПоИдентификатору(стр.ИдентификаторУзелаДерева);
		Если Узел.Действие="ТестовыйСлучай" Тогда
			ТестРезультатСтруктура.ТестовыеСлучаи = новый Массив;	
		КонецЕсли;
	КонецЕсли; 	
...
Функция ВыгрузитьСценарийФорматXML(ЛогОшибок=Ложь)
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(Объект.НаименованиеНабораСценариев) Тогда
			Объект.НаименованиеНабораСценариев = "Без названия";	
		КонецЕсли;
		НоваяЗаписьXML.ЗаписатьАтрибут("НаборСценариев",Объект.НаименованиеНабораСценариев);
		...
		Если НЕ ЗначениеЗаполнено(Объект.GUID) Тогда
			Объект.GUID = строка(новый uuid());
		КонецЕсли;
		НоваяЗаписьXML.ЗаписатьАтрибут("GUID",Объект.GUID);
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.Имя = "Скрипт" Тогда			
			Если ЗагрузкаБлока=Ложь Тогда
				...
				Попытка Объект.НаименованиеНабораСценариев = ЧтениеXML.ПолучитьАтрибут("НаборСценариев");Исключение КонецПопытки; // совместимость
				Объект.Автор = ЧтениеXML.ПолучитьАтрибут("Автор");
				Объект.АдресИнтернет = ЧтениеXML.ПолучитьАтрибут("АдресИнтернет");
				Попытка Объект.GUID = ЧтениеXML.ПолучитьАтрибут("GUID"); Исключение КонецПопытки; // совместимость
				Попытка 
					Объект.ИдентификаторТеста = ЧтениеXML.ПолучитьАтрибут("ИдентификаторТеста"); 
				Исключение 
					Объект.ИдентификаторТеста = СформироватьАвтоматическиИдентификаторТеста(Объект.Наименование);
				КонецПопытки; // совместимость								
				Попытка Объект.Severity = ЧтениеXML.ПолучитьАтрибут("Severity"); Исключение КонецПопытки; // совместимость
			КонецЕсли;
...
Процедура ЗагрузитьСценарийФорматXMLСвойстваШага(ЧтениеXML,ШагСвойства)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ИмяТега = "Свойства" И ЧтениеXML.ТипУзла=ТипУзлаXML.КонецЭлемента Тогда
			Возврат;
		ИначеЕсли Найти("Активность ПродолжитьПриОшибке ЭтоГлавноеОкно ЭтоБлок SwitchSelection Cancel ИспользоватьВариантыПоиска",ИмяТега) 
		...
		ИначеЕсли Найти("Наименование Описание Library ПутьБлокШагов ИмяПараметра ИмяПараметра2 Area УсловиеСравнения
			...
			| OutputText CommandRef Presentation Direction RowDescription ПредставлениеДанных СвойстваПодключенияКлиентаТестирования",ИмяТега) 
			И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
			ЧтениеXML.Прочитать();
...
Процедура ТаблицаСвойстваПриАктивизацииСтроки(Элемент)
	...
	Если Элементы.ТаблицаСвойстваЗначение.Вид = ВидПоляФормы.ПолеВвода Тогда
		Если ТекущиеДанные.Свойство="Действие" ИЛИ ТекущиеДанные.Свойство="Команда" ИЛИ ТекущиеДанные.Свойство="ТипОбъекта" 
			ИЛИ ТекущиеДанные.Свойство="Severity" Тогда
		...
		ИначеЕсли ТекущиеДанные.Свойство="УсловиеСравнения" Тогда
			Элементы.ТаблицаСвойстваЗначение.КнопкаВыбора = Истина;
			мСценРедактор_УстановитьДействие("ТаблицаСвойстваЗначение","НачалоВыбора","мСценРедактор_ВнешняяФормаВыбора");
		ИначеЕсли ТекущиеДанные.Свойство="ИмяПараметра" Тогда
			Элементы.ТаблицаСвойстваЗначение.КнопкаВыбора = Истина;
			мСценРедактор_УстановитьДействие("ТаблицаСвойстваЗначение","НачалоВыбора","мСценРедактор_ВнешняяФормаВыбора");
...
Процедура мСценРедактор_ВнешняяФормаВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Попытка
		Если Элемент.Родитель.ТекущиеДанные.Свойство="ИмяПараметра" Тогда
...
&НаСервереБезКонтекста
Функция мСценРедактор_ПолучитьСписокСвойствДляДействияКоманды(Знач Действие="", Знач Команда="")
	...
	ИначеЕсли Действие="Команда" Тогда
		...
		Если Команда="ВвестиТекст" Тогда
...
Процедура мСценРедактор_ИзменитьВидимостьДоступностьТаблицыСвойств(ТекущиеДанные)
	...
	ДоступныеСвойстваМассив = мСценРедактор_ПолучитьСписокСвойствДляДействияКоманды(Действие,Команда);
...
Функция мСценРедактор_СкопироватьСтрокуДерева(РеквизитДерево, Приемник, Источник, УдалятьИзИсточника=Истина)
	...
	НоваяСтрока.UID = XMLСтрока(новый UUID());
...
Процедура мСценСкрипт_СделатьШагПоТаблицеНаборовСценариевНачало()
	...
	Если ИндексТекущегоТеста=0 Тогда
		ВремяНачалаТеста = ТекущаяДата();
		ВремяВыполненияUTC = ТекущаяУниверсальнаяДатаВМиллисекундах();
	КонецЕсли;
...
Процедура ЗавершитьТестовыеПриложенияПринудительно(ЗакрытьВсе=Ложь)
	...
	Если ИспользоватьКомандуЗапуститьСистему=Истина ИЛИ ЗакрытьВсе=Истина Тогда
		плагин_ЗавершитьВсеТестовыеПриложенияПринудительно();
	Иначе
		Для каждого стр из ПуллТестируемыхПриложений Цикл
			плагин_ЗавершитьПроцессЛокально(стр.Значение,Сообщение);
		КонецЦикла;	
	КонецЕсли;
...
&НаКлиенте
Процедура ЗапуститьТестовыеПриложения(СтрокаЗапуска,НомерПорта=0)
	...
	Иначе
		ПИД = плагин_ЗапуситьПриложениеЛокально(СтрокаЗапуска);
		ПуллТестируемыхПриложений.Вставить(НомерПорта,ПИД);
	КонецЕсли;
...
Процедура мСценЖурнал_НачатьЗапись(Команда)
	...
	Попытка
		Если ТестовоеПриложениеЖурнал=Неопределено Тогда
			мСценСкрипт_GenerateClientConnection(ТестовоеПриложениеЖурнал,ОписаниеОшибки,новый Структура);
...
Процедура мАгент_ВыполнитьОбработкуЗапроса()
	...
	Агент_СтруктураОтвета = новый Структура("ИмяКоманды,НомерСообщения","Неопределено",0);
	...
	Если Агент_СтруктураЗапроса.ТипСообщения="command" Тогда
		...
		ИначеЕсли Агент_СтруктураЗапроса.Наименование="ЗапуститьКлиентаТестирования" Тогда
			...
			мСценСкрипт_ЗапуститьКлиентовТестированияДляТеста(Истина);
			Агент_СтруктураОтвета.Вставить("ИмяКоманды","success");
...
&НаКлиенте
Процедура ТипТестируемогоКлиентаПриИзменении(Элемент)
	// Вставить содержимое обработчика.
КонецПроцедуры
