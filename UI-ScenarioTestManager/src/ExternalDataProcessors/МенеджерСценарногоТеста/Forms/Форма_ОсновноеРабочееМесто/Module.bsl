...
Процедура ВыделениеИЗаменаВСценарииШаблонов(Знач мПараметр)
	...
	Для каждого шаблон из мПараметр.МассивСтруктурЗамен Цикл
		...
		Для ш=шаблон.Начало по шаблон.Окончание Цикл
			...
			Если УзелВхода.ЭтоБлок=Истина И УзелВхода.Действие="ГотовыйБлокШагов" Тогда
...
//		СоответсвиеДляПоиска.Получить("Goal").Видимость = Истина;
	...
	ИначеЕсли Действие="Команда" Тогда
		...
		Если (API="AutomationUI" ИЛИ API="Selenium") И (Найти(Команда,"attribute") ИЛИ Найти(Команда,"get text") ИЛИ Найти(Команда,"get window title") 
