...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ПутьКФайлуExternAutomationUI) Тогда
		Объект.ПутьКФайлуExternAutomationUI = "http://localhost"
	КонецЕсли;  
			...
			Попытка
				...
				Если ТипЗнч(ОписаниеПодключения)=Тип("Соответствие") Тогда
					Если ОписаниеПодключения.Получить("НастройкиПоУмолчанию")=Ложь Тогда						
						Браузер = ОписаниеПодключения.Получить("Браузер");
						API = ОписаниеПодключения.Получить("API"); 
	...
	Если ИспользоватьКомандуЗапуститьСистему = Истина Тогда
		Для каждого стр из ПуллТестируемыхПриложений Цикл
			...
			ИначеЕсли ПараметрыПриложения.API = "Selenium" Тогда
				Модуль_ЗаписьЖурналаДействий.ЗакрытьБраузер_Selenium_API(Объект.НомерПортаExternAutomationUI, ПараметрыПриложения.НомерПорта);
			КонецЕсли;
		КонецЦикла;
	Иначе
		Для каждого стр из ПуллТестируемыхПриложений Цикл
			ПараметрыПриложения = стр.Значение;
			Если ПараметрыПриложения.API = "" ИЛИ ПараметрыПриложения.API = "1cClientApp"
...
Процедура ЗапуститьТестовыеПриложения(Знач СтрокаЗапуска,Знач НомерПорта=0,Знач API="1cClientApp",Знач Браузер="Chrome")
	...
	Если API="Selenium" Тогда // Запускаем браузеры
		Модуль_ЗаписьЖурналаДействий.ЗапуститьБраузер_Selenium_API(Браузер,Объект.НомерПортаExternAutomationUI,НомерПорта);
...
Процедура мСценЖурнал_НачатьЗапись(Команда)	
	...
	ИначеЕсли Объект.ИспользуемыйAPI="Selenium" Тогда
		Модуль_ЗаписьЖурналаДействий.НачатьЗапись_Selenium_API(Объект.НомерПортаExternAutomationUI,Объект.НомерПорта);
		Элементы.мСценЖурнал_ЗавершитьЗапись.Доступность = Истина;
		Элементы.мСценЖурнал_ПрерватьЗапись.Доступность = Истина;
		Элементы.мСценЖурнал_НачатьЗапись.Видимость = Ложь;
		Элементы.мСценЖурнал_ПаузаЗапись.Видимость = Истина;
...
Процедура мСценЖурнал_ЗавершитьЗапись(Команда)
	...
	ИначеЕсли Объект.ИспользуемыйAPI="Selenium" Тогда
		ИсточникТекст = Модуль_ЗаписьЖурналаДействий.ЗавершитьЗапись_Selenium_API(Объект.НомерПортаExternAutomationUI,Объект.НомерПорта);
	КонецЕсли;	
