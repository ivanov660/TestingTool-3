...
Процедура ВыделениеИЗаменаВСценарииШаблонов(Знач мПараметр)
	...
	Для каждого шаблон из мПараметр Цикл
		...
		//Узел = УзелВхода.ПолучитьРодителя().ПолучитьЭлементы().Добавить();
		Если УзелВхода.ПолучитьРодителя()=Неопределено Тогда
...
Процедура мСценСкрипт_ПоискПодобияШаблона(Команда)
	мПараметры = Новый Структура("ПутьККаталогуБиблиотекиСценариев",Объект.ПутьККаталогуБиблиотекиСценариев);
