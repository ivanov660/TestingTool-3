...
Процедура СоздатьСкриншот(Узел,Режим,ТекстОшибки)
	...
	ПутьКФайлу = Объект.ПутьККаталогуОтчетовВыполненияТестов+"\"+КаталогКартинок+"\"+ИмяСценарияДляФайла+"ScreenShot-Step-"+Формат(Узел.НомерШага,"ЧГ=;")+".jpg";
...
Функция мСценРедактор_ПолучитьСписокСвойствДляДействияКоманды(Знач Действие="", Знач Команда="",знач API="")
	...
	ИначеЕсли Действие="Команда" Тогда
		...
		Если (API="AutomationUI" ИЛИ API="Selenium") И (Найти(Команда,"attribute") ИЛИ Найти(Команда,"get text")) Тогда
			СоответсвиеДляПоиска.Получить("Attribute").Видимость = Истина;			
		КонецЕсли;
