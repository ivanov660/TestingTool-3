...
Процедура СоздатьСкриншот(Узел,Режим,ТекстОшибки)
	...
	Иначе
		ПутьКФайлу = Объект.ПутьККаталогуОтчетовВыполненияТестов+"\"+ИмяСценарияДляФайла+"ScreenShot-Step-"+Формат(Узел.НомерШага,"ЧГ=;")+".jpg";
	КонецЕсли;
...
Функция ВыгрузитьСценарийФорматXML(ЛогОшибок=Ложь)
	...
	Попытка
		...
		НоваяЗаписьXML.ЗаписатьАтрибут("Версия","2.1");
