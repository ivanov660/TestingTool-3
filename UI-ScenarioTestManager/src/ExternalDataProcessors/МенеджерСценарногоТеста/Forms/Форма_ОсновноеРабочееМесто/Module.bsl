...
&НаКлиенте
Перем Модуль_ОбработкаДействийSelenuim;
...
Процедура ПриОткрытии(Отказ)
	...
	Модуль_ОбработкаДействийSelenuim = ПолучитьФорму("ВнешняяОбработка.МенеджерСценарногоТеста.Форма.Модуль_ОбработкаДействийSelenuim");
...
Процедура мСценСкрипт_ОбработатьТипУзла(Узел,ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,мПараметрыКлиентСервер)
	...
	ИначеЕсли ТекущийAPI = "Selenuim" Тогда
		ТекущийМодульОбработки = Модуль_ОбработкаДействийSelenuim;
	Иначе
	...
	Если Узел.Действие="ПодключитьТестируемоеПриложение" ИЛИ Узел.Действие="GenerateClientConnectionScript" Тогда
		...
		ТекущийМодульОбработки.мСценСкрипт_GenerateClientConnection(ТестовоеПриложение,ОписаниеОшибки,Узел.СвойстваПодключенияКлиентаТестирования,ПараметрыСценария,Объект.НомерПорта,Узел.Интервал);
		
	ИначеЕсли Узел.Действие="GenerateClientDisconnectionScript" ИЛИ Узел.Действие="ЗакрытьТестируемоеПриложение" Тогда
		
		ТекущийМодульОбработки.мСценСкрипт_GenerateClientDisconnection(ТестовоеПриложение,ОписаниеОшибки,Узел.СвойстваПодключенияКлиентаТестирования);
	...
	ИначеЕсли Узел.Действие = "ПолучитьПредставлениеДанных" Тогда
		...
		ТекущийМодульОбработки.мСценСкрипт_ПолучитьПредставлениеДанных(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,ТекущаяПеременная,Узел.ПредставлениеДанных,Узел.ИмяПараметра,Узел.ЗначениеПараметра,ПараметрыСценария);
		
	ИначеЕсли Узел.Действие = "СравнитьСПредставлениемДанных" Тогда
		
		ТекущийМодульОбработки.мСценСкрипт_СравнитьСПредставлениемДанных(ТестовоеПриложение,ОписаниеОшибки,мПараметрыКлиент,ТекущаяПеременная,Узел.ПредставлениеДанных,ПараметрыСценария,Узел.УсловиеСравнения);
	...
	ИначеЕсли Узел.Действие="НайтиОсновноеОкно" Тогда
		...
		ТекущийМодульОбработки.мСценСкрипт_НайтиОсновноеОкно(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная);
		
	ИначеЕсли Узел.Действие="НайтиОкно" ИЛИ Узел.Действие="FindWindow" Тогда
		
		ТекущийМодульОбработки.мСценСкрипт_НайтиОкно(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная,ЗаголовокОбъекта,Узел.ИспользоватьВариантыПоиска);
		
	ИначеЕсли Узел.Действие="НайтиФорму" ИЛИ Узел.Действие="FindForm" Тогда	
		
		ТекущийМодульОбработки.мСценСкрипт_НайтиФорму(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная,РодительПеременная,ЗаголовокОбъекта,Узел.ИспользоватьВариантыПоиска);
	...
	ИначеЕсли Узел.Действие = "НайтиОбъект" ИЛИ Узел.Действие = "FindObject" Тогда
		...
		ТекущийМодульОбработки.мСценСкрипт_НайтиОбъект(ТестовоеПриложение,ОписаниеОшибки,Узел.ТипОбъекта,ТекущаяПеременная,РодительПеременная,ЗаголовокОбъекта,Узел.ИмяОбъекта,Узел.ИмяКлассаОбъекта,Узел.ИспользоватьВариантыПоиска);
		
	ИначеЕсли Узел.Действие = "ПроверкаНаличияЭлемента" Тогда
	
		ТекущийМодульОбработки.мСценСкрипт_ПроверкаНаличияЭлемента(ТестовоеПриложение,ОписаниеОшибки,Узел.ТипОбъекта,ЗаголовокОбъекта,Узел.ИмяОбъекта,Узел.ИмяПараметра,ПараметрыСценария,Узел.ИспользоватьВариантыПоиска);
	...
	ИначеЕсли Узел.Действие = "Команда" ИЛИ Узел.Действие = "Command" Тогда
		...
		ТекущийМодульОбработки.мСценСкрипт_ВыполнитьКоманду(ТестовоеПриложение,ОписаниеОшибки,Узел.Команда,Узел.ТипОбъекта,ТекущаяПеременная,РодительПеременная,ЗаголовокОбъекта,OutputText,Узел.CommandRef,Presentation,Узел.Direction,Узел.RowDescription,Узел.SwitchSelection,Узел.Cancel,Узел.Area,Узел.ИмяПараметра,Узел.ЗначениеПараметра,ПараметрыСценария);
...
Процедура мСценЖурнал_НачатьЗапись_1cClientApp_API() Экспорт
	...
	ОписаниеОшибки = "";		
		Если ТестовоеПриложениеЖурнал=Неопределено Тогда
			Модуль_ОбработкаДействий.мСценСкрипт_GenerateClientConnection(ТестовоеПриложениеЖурнал,ОписаниеОшибки,новый Структура,ПараметрыСценария,Объект.НомерПорта);
