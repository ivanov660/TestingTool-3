...
Функция мСценСкрипт_ПолучитьДетальныйРезультатВыполненияСценария(ВыгружатьШаги=Ложь)
	...
	Для ш=0 по ТаблицаОтчетаСценария.Количество() Цикл
		...
		Если ТекущийНомерШагаDDT<>стр.НомерШагаDDT ИЛИ ш=ТаблицаОтчетаСценария.Количество()-1 Тогда
			TestCase = мСценСкрипт_ПолучитьДетальныйРезультатВыполненияСценарияПоМассивуСтруктур(МассивСтруктур,ТекущийНомерШагаDDT);
...
&НаКлиенте
Функция мСценСкрипт_ПолучитьДетальныйРезультатВыполненияСценарияПоМассивуСтруктур(Знач МассивСтруктур,Знач ТекущийНомерШагаDDT)
