...
Процедура ВыделениеИЗаменаВСценарииШаблонов(Знач мПараметр)
	...
	СформироватьКешСтруктурДереваСценария(ДеревоСценария,1,КешСоответсивяНумерацииУзламДерева);
	
	// 1. это эквивалентно вставке в позицию блока библиотеки
	Для каждого шаблон из мПараметр Цикл
		...
		Если УзелВхода=Неопределено Тогда
			// почему-то не нашелся узел входа, возможно другой сенарий
		...
		Если НЕ ЗагрузитьСценарийФорматXML(Узел.ПутьБлокШагов,Истина) = Ложь Тогда
	...
	Для каждого шаблон из мПараметр Цикл
		...
		Для ш=шаблон.Начало по шаблон.Окончание Цикл
			...
			Если УзелВхода.ПолучитьЭлементы().Количество()=0 Тогда
...
&НаКлиенте
Процедура СформироватьКешСтруктурДереваСценария(Знач УзелРодитель, Позиция=1, КешСоответсивяНумерацииУзламДерева)
	...
	Для каждого Узел из УзелРодитель.ПолучитьЭлементы() Цикл
		...
		// в иерархию
		СформироватьКешСтруктурДереваСценария(Узел, Позиция, КешСоответсивяНумерацииУзламДерева);
...
Функция ВыгрузитьСценарийФорматXML(ЛогОшибок=Ложь)
	...
	Попытка
		...
		НоваяЗаписьXML.ЗаписатьАтрибут("Count",Строка(ТаблицаПоддерживаемыеВерсииТК.Количество()));
		Для каждого стр из ТаблицаПоддерживаемыеВерсииТК Цикл
		...
		НоваяЗаписьXML.ЗаписатьТекст(Логика_Action);
		НоваяЗаписьXML.ЗаписатьКонецЭлемента(); 
		
		НоваяЗаписьXML.ЗаписатьНачалоЭлемента("Description");
		НоваяЗаписьXML.ЗаписатьТекст(Логика_Description);
		НоваяЗаписьXML.ЗаписатьКонецЭлемента();
		
		НоваяЗаписьXML.ЗаписатьНачалоЭлемента("Cost");
		НоваяЗаписьXML.ЗаписатьТекст(XMLСтрока(Логика_Cost));
		НоваяЗаписьXML.ЗаписатьКонецЭлемента(); 
		
		// Preconditions
		НоваяЗаписьXML.ЗаписатьНачалоЭлемента("Preconditions");
		НоваяЗаписьXML.ЗаписатьАтрибут("Count",Строка(Логика_preconditions.Количество()));
		...
		НоваяЗаписьXML.ЗаписатьАтрибут("Count",Строка(Логика_effects.Количество()));
...
Функция ЗагрузитьСценарийИлиБлокФорматXML(Знач ПутьКфайлу="",Знач ЗагрузкаБлока=Ложь,UIDКорня="00000000-0000-0000-0000-000000000000",BFUIDКорня="00000000-0000-0000-0000-000000000000")  Экспорт
	...
	Если ЗагрузкаБлока=Ложь Тогда
		Логика_Action = "";
		Логика_Description = "";
		Логика_preconditions.Очистить();
		Логика_effects.Очистить();
		ТаблицаПоддерживаемыеВерсииТК.Очистить();
	КонецЕсли;
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		ИначеЕсли ЧтениеXML.Имя = "SupportedVersions" Тогда
			Если ЗагрузкаБлока=Ложь Тогда
				ЗагрузитьСценарийФорматXMLSupportedVersions(ЧтениеXML);
			КонецЕсли;
		ИначеЕсли ЧтениеXML.Имя = "Logic" Тогда
			Если ЗагрузкаБлока=Ложь Тогда
				ЗагрузитьСценарийФорматXMLLogic(ЧтениеXML);
			КонецЕсли;
...
&НаКлиенте
Процедура ЗагрузитьСценарийФорматXMLLogic(ЧтениеXML) Экспорт
		...
		ИначеЕсли ЧтениеXML.Имя = "Preconditions" Тогда
			Пока ЧтениеXML.Прочитать() Цикл
				...
				ИначеЕсли ЧтениеXML.Имя = "Item" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
					стр_precondition = Логика_preconditions.Добавить();		
		...
		ИначеЕсли ЧтениеXML.Имя = "Effects" Тогда
			Пока ЧтениеXML.Прочитать() Цикл
				Если ЧтениеXML.Имя = "Effects" И ЧтениеXML.ТипУзла=ТипУзлаXML.КонецЭлемента Тогда
					Прервать;			
				ИначеЕсли ЧтениеXML.Имя = "Item" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
					стр_effect = Логика_effects.Добавить();		
				ИначеЕсли ЧтениеXML.Имя = "Variable" И ЧтениеXML.ТипУзла=ТипУзлаXML.НачалоЭлемента Тогда
					ЧтениеXML.Прочитать();
...
&НаКлиенте
Процедура ЗагрузитьСценарийФорматXMLSupportedVersions(ЧтениеXML) Экспорт
	...
	Пока ЧтениеXML.Прочитать() Цикл
		Если ЧтениеXML.Имя = "SupportedVersions" И ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
			Возврат;
		ИначеЕсли ЧтениеXML.Имя = "Item" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			стр_vs = ТаблицаПоддерживаемыеВерсииТК.Добавить();
...
&НаКлиенте
Функция ПреобразоватьДеревоСценарияВТекст() Экспорт
	Возврат ПреобразоватьДеревоТекст(ДеревоСценария,"",Ложь);
КонецФункции

&НаКлиенте
Функция ПреобразоватьДеревоТекст(УзелРодитель,Отступ="",РежимHTML=Истина) Экспорт
	...
	Для каждого Узел из УзелРодитель.ПолучитьЭлементы() Цикл
		Если РежимHTML=Истина Тогда
			...
			Текст = Текст+ПреобразоватьДеревоТекст(Узел,Отступ,РежимHTML);
			Текст = Текст+"</li>";
		Иначе
...
Процедура мСценСкрипт_ПреобразоватьДеревоТекст(Команда)
	Текст = ПреобразоватьДеревоТекст(ДеревоСценария);
