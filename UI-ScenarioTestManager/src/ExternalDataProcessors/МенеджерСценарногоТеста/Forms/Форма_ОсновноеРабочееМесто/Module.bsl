...
Процедура СоздатьСкриншот(Узел,Режим,ТекстОшибки)
	...
	Если ПараметрыТекущегоПриложения.PID=0 Тогда
		Возврат;
	КонецЕсли;
	
	КаталогКартинок = "ScreenShot";//Объект.ИмяЛога;
	
	ПутьКФайлу = Объект.ПутьККаталогуОтчетовВыполненияТестов+"\"+КаталогКартинок+"\"+Объект.ИмяЛога+"ScreenShot-Step-"+Формат(Узел.НомерШага,"ЧГ=;")+".jpg";
...
Функция мСценСкрипт_ПолучитьПараметрыТекущегоПриложения(Узел)
	...
	ПараметрыТекущегоПриложения = новый Структура("PID",0);
	
		
	// определеим ПИД
	Если ТекущийAPI = "AutomationUI" ИЛИ ТекущийAPI = "" ИЛИ ТекущийAPI = "1cClientApp" Тогда
...
Функция мСценСкрипт_ПолучитьДетальныйРезультатВыполненияСценария(ВыгружатьШаги=Ложь)
	...
	Для каждого стр из ТаблицаСценария Цикл
		...
		СтруктураШага = новый Структура("Шаги,name,title,start,stop,РезультатВыполнения,ТекстОшибки,НомерШага,Действие,Команда,severity,status,Attachments",новый Массив,
			Узел.Наименование,Узел.Наименование,Узел.UTC,Узел.UTC,Узел.РезультатВыполнения,Узел.ТекстОшибки,Узел.НомерШага,Узел.Действие,Узел.Команда,Узел.Severity,Неопределено,новый Массив);
		Если ТипЗнч(Узел.Attachments)=Тип("СписокЗначений") И Узел.Attachments.Количество()>0 Тогда
			Для каждого влож из Узел.Attachments Цикл
...
Функция мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандAutomationUI()
	...
	// Отправить текст
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Отправить текст");
...
Функция мСценРедактор_ПолучитьМассивСтруктурДоступныхКомандSelenium()
	...
	// Перейти по адресу
	стр_н = новый Структура;
	стр_н.Вставить("Синоним","Implicit wait");
...
Процедура мСценРедактор_ИзменитьПодсказкуTips(ТекущиеДанные)
	...
	ИначеЕсли ТекущиеДанные.Действие="Команда" Тогда
		...
		Если Объект.ИспользуемыйAPI="Selenium" Тогда
			...
			КонецЕсли;
		ИначеЕсли Объект.ИспользуемыйAPI="AutomationUI" Тогда
			Если НРЕГ(ТекущиеДанные.Команда)="click" ИЛИ НРЕГ(ТекущиеДанные.Команда)="dblclick" Тогда
				ТекстПодсказки = ТекстПодсказки+"<br /> <b>click</b> или <b>dblclick</b> - одинарный или двойной клик";
			ИначеЕсли НРЕГ(ТекущиеДанные.Команда)="focus" Тогда
				ТекстПодсказки = ТекстПодсказки+"<br /> <b>focus</b> - устанавливает фокус на элемент";
			...
			ИначеЕсли НРЕГ(ТекущиеДанные.Команда)="text" Тогда
				ТекстПодсказки = ТекстПодсказки+"<br /> <b>text</b> - вводит текст на выбранном элементе.";
			ИначеЕсли НРЕГ(ТекущиеДанные.Команда)="send key" Тогда
				ТекстПодсказки = ТекстПодсказки+"<br /> <b>send key</b> - отправляет команду-текст на выбранном элементе. Чтобы отправить горячую клавишу F4 - введите '{F4}'. Используйте перед focus.";		
