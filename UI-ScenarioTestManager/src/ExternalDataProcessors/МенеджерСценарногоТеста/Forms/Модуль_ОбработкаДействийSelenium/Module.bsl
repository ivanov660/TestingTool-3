...
Процедура мСценСкрипт_НайтиОбъект(ТестовоеПриложение,ОписаниеОшибки,ТекущаяПеременная, мПараметры) Экспорт
	...
	СтрокаКоманды = СтрокаКоманды + "&element_title="+Модуль_СервисныеФункции.EncodeURL(ЗаголовокОбъекта);
...
Процедура мСценСкрипт_ПроверкаНаличияЭлемента(ТестовоеПриложение, ОписаниеОшибки, мПараметры, ПараметрыСценария) Экспорт
	...
	Попытка
		мСценСкрипт_НайтиОбъект(ТестовоеПриложение,ОписаниеОшибки,Неопределено,мПараметры);
		ОбъектНайден = Истина;
	Исключение
		ОписаниеОшибки = "";
		// увы не нашли
