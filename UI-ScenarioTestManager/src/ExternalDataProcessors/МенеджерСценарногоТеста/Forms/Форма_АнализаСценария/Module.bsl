...
Процедура ИнициироватьВероятностнуюСеть(Команда)
	...
	ТекстШаблонаБезПлохихСимволов = ОчиститьТекстОтплохихСимволов(ТекстШаблона);
	...
	мНастройка = Модуль_ОбработкаТекста.СформироватьСтруктуруНастроекПоУмолчанию();
	мНастройка.Текст_ИспользоватьСтеммерПортера=Ложь; // не нужен стеммер, пока
	мНастройка.Текст_УдалятьНомераДокументов=Истина; // не нужен стеммер, пока
	мНастройка.Вставить("Вариант","КоличествоРазНабор");
...
Процедура ПолучитьТекстДереваСценария(Команда)
	ТекстСценария = ЭтаФорма.ВладелецФормы.ПреобразоватьДеревоСценарияВТекст();
...
Процедура ПроставитьЗамены(Команда)
	...
	// Оповещаем
	Оповестить("ПроставитьШаблоныЗамены",МассивСтруктурЗамен,ЭтаФорма);
...
Процедура ЗагрузитьШаблоны(Команда)
	...
	Для каждого стр_файл из МассивФайлов Цикл
		...
		Если Структура=Неопределено Тогда
			Структура = Новый Структура("Шаблон,ТекстыШаблонов,Действие,ПутьШаблон","","","","");
		КонецЕсли;
...
Процедура СохранитьШаблоны(Команда)
	...
	// Преобразуем тексты в таблицу
	Если ТаблицаСвойствШаблонов.Количество()=0 Тогда 
		
		Текст = СтрЗаменить(ТекстШаблона,"end_of_text_paragraf",Символ(18));
		МассивШаблонов = СтрРазделить(Текст,Символ(18),Ложь);
		
		МассивШаблонов = ПреобразоватьТекстМассивСтруктурМаркированныхДанных(ТекстШаблона,Ложь);
		СоответствиеШаблонов = Новый Соответствие;
		Для каждого стр из МассивШаблонов Цикл
			Если стр.Заголовок="" Тогда
				Продолжить;
			КонецЕсли;
			Структура = СоответствиеШаблонов.Получить(стр.Заголовок);
			Если Структура=Неопределено Тогда
				Структура = Новый Структура("Шаблон,ТекстыШаблонов",стр.Заголовок,"");
			КонецЕсли;                                                                       
			
			Если Структура.ТекстыШаблонов="" Тогда
				Структура.ТекстыШаблонов = стр.Текст+Символы.ПС+Символы.ПС+"end_of_text_paragraf"+Символы.ПС;
			Иначе
				Структура.ТекстыШаблонов = Структура.ТекстыШаблонов+Символы.ПС+стр.Текст+Символы.ПС+Символы.ПС+"end_of_text_paragraf"+Символы.ПС;
			КонецЕсли;
			
			СоответствиеШаблонов.Вставить(стр.Заголовок,Структура);
...
Процедура ДобавитьВОбучающуюВыборку(Команда)
	...
	Если ИмяШаблонаНовыйФрагмент="" Тогда
		...
		Возврат;
	КонецЕсли;
	ТекстШаблона = ТекстШаблона+Символы.ПС+Символы.ПС+"name_of_pattern "+ИмяШаблонаНовыйФрагмент+Символы.ПС+Символы.ПС+НовыйФрагментШаблона+Символы.ПС+Символы.ПС+"end_of_text_paragraf"+Символы.ПС;
