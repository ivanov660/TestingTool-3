...
Функция ПрочитатьПараметрыИзБазы(Знач Ключ, Знач ПараметрыСтрокой) Экспорт
	...
	Для каждого стр из МассивПараметров Цикл
		...
		Попытка		
			Выполнить("ЗначениеПараметра = ПланировщикЗаданийВызовСервера.ПолучитьЗначениеПользовательскойПеременной(Неопределено,ИмяПараметра,Ключ);");
		Исключение
...
Процедура ЗаписатьПараметрыВБазу(Знач Ключ, Знач СоответсвиеДанных) Экспорт
	...
	Для каждого стр из СоответсвиеДанных Цикл
		...
		Попытка		
			выполнить("ПланировщикЗаданийВызовСервера.УстановитьЗначениеПользовательскойПеременной(Неопределено,ИмяПараметра,ЗначениеПараметра,Ключ);");
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			ВызватьИсключение ТекстОшибки;
		Конецпопытки;
...
Процедура УдалитьПараметрыИзБазы(Знач Ключ, Знач ПараметрыСтрокой) Экспорт
	...
	Для каждого стр из МассивПараметров Цикл
		ИмяПараметра = СокрЛП(стр);
		Попытка		
			Выполнить("ПланировщикЗаданийВызовСервера.УдалитьЗначениеПользовательскойПеременной(Неопределено,ИмяПараметра,Ключ);");
		Исключение
			ТекстОшибки = ОписаниеОшибки();
			ВызватьИсключение ТекстОшибки;
		Конецпопытки;		
...
Функция ПрочитатьПараметрыПодключения(Знач Ключ, Знач ПараметрыСтрокой, Знач ПараметрыПодключенияПоУмолчанию) Экспорт
	...
	СтруктураДанных = Неопределено;
	СтруктураПодключения = Неопределено;
	МассивПараметров = СтрРазделить(ПараметрыСтрокой,",;:/\",Ложь);
	...
	ИмяПараметра = СокрЛП(ПараметрыСтрокой);
	
	Попытка	
		Выполнить("СтруктураПодключения = ПланировщикЗаданийВызовСервера.ПолучитьДанныеПодключенияПользователяПоКлючу(Ключ,ИмяПараметра);");
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		ВызватьИсключение ТекстОшибки;
	Конецпопытки;
...
Функция ПрочитатьПараметрыСценариевИзБазы(Знач Ключ, Знач ПараметрыСтрокой) Экспорт
	...
	КонецЕсли;
	// удалим пробелы
	НормированнаяСтрокаПараметров="";
	// НормированнаяСтрокаПараметров = СтрСоединить(МассивПараметров,",");
	Для каждого стр из МассивПараметров Цикл
		НормированнаяСтрокаПараметров = НормированнаяСтрокаПараметров +?(НормированнаяСтрокаПараметров="","",",") + СокрЛП(стр);
	КонецЦикла;
	
	Попытка
		Выполнить("МассивДанных = СценарноеТестированиеВызовСервера.ПолучитьДанныеПараметровСценариевПоКлючу(Ключ,НормированнаяСтрокаПараметров);");
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		ВызватьИсключение ТекстОшибки;
	Конецпопытки;
	...
	Для каждого стр из МассивПараметров Цикл
		Если СоответствиеДанных.Получить(СокрЛП(стр))=Неопределено Тогда
