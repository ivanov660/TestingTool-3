...
&НаКлиенте
Процедура НачатьЗапись_Selenium_API(Знач PID=0,Знач НомерПорта=8080) Экспорт
	...
	КонецЕсли;
	// 3. Отправляем команду начала записи 
	СтрокаКоманды = "&Operation=record_run";
...
&НаКлиенте
Процедура ПаузаЗапись_Selenium_API(Знач НомерПорта=8080) Экспорт
...
&НаКлиенте
Функция ЗавершитьЗапись_Selenium_API(Знач НомерПорта=8080) Экспорт
...
&НаКлиенте
Функция ЗапуститьБраузер_Selenium_API(Знач Браузер,Знач НомерПорта=8080) Экспорт
...
&НаКлиенте
Функция ЗакрытьБраузер_Selenium_API(Знач Браузер,Знач НомерПорта=8080) Экспорт
