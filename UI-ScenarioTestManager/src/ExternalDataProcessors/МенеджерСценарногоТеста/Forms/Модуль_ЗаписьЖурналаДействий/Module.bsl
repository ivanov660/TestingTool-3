...
Процедура НачатьЗапись_AutomationUI_API(Знач PID=0) Экспорт
	...
	СтруктураОтвета = ЗагрузитьФайлПоИнтернетАдресу("http://localhost:8080/rest.html");
	...
	СтруктураОтвета = ЗагрузитьФайлПоИнтернетАдресу("http://localhost:8080/rest.html?&Action=record_run"+?(PID<>0,"&process_id="+Формат(PID,"ЧГ=;"),""));
	Если НЕ СтруктураОтвета.КодСостояния=200 Тогда
...
Процедура ПаузаЗапись_AutomationUI_API() Экспорт
	...
	СтруктураОтвета = ЗагрузитьФайлПоИнтернетАдресу("http://localhost:8080/rest.html?&Action=record_pause");
	Если НЕ СтруктураОтвета.КодСостояния=200 Тогда
...
Функция ЗавершитьЗапись_AutomationUI_API() Экспорт
	...
	СтруктураОтвета = ЗагрузитьФайлПоИнтернетАдресу("http://localhost:8080/rest.html?&Action=record_stop");
	Если НЕ СтруктураОтвета.КодСостояния=200 Тогда
...
Функция СписокПриложенийДляОтладки_AutomationUI_API() Экспорт
	...
	СтруктураОтвета = ЗагрузитьФайлПоИнтернетАдресу("http://localhost:8080/rest.html?&Action=app_list");
...
Функция ЗагрузитьФайлПоИнтернетАдресу(Знач ПолныйАдресРесурса)
	...
	Исключение
		...
		СтруктураОтвета.ТекстОшибки = "Произошла сетевая ошибка!"+Символы.ПС+ОписаниеОшибки();
	...
	Если Результат.КодСостояния >= 400 и Результат.КодСостояния < 500  Тогда
		СтруктураОтвета.ТекстОшибки = "Код статуса больше 4XX, ошибка запроса.  Код статуса: " + Результат.КодСостояния;
		Возврат СтруктураОтвета;
	КонецЕсли;
	...
	Если Результат.КодСостояния >= 500 и Результат.КодСостояния < 600  Тогда
		СтруктураОтвета.ТекстОшибки = "Код статуса больше 5XX, ошибка сервера. Код статуса: " + Результат.КодСостояния;
		Возврат СтруктураОтвета;
	КонецЕсли;
	...
	Если Результат.КодСостояния >= 300 и Результат.КодСостояния < 400  Тогда
		...
		Если Результат.КодСостояния = 302 Тогда
			...
			Иначе
				СтруктураОтвета.ТекстОшибки = "Код статуса больше 3XX, Перенаправление. Код статуса: " + Результат.КодСостояния+". Сервер не сообщил адрес ресурса!";
				Возврат СтруктураОтвета;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
