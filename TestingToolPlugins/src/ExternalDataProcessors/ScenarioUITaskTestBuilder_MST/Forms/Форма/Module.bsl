...
Процедура НайтиПараметрыПоЗаданию()
	...
	|	И ПеременныеЗаданий.ЭтоПараметрНастройки = ИСТИНА";
	Запрос.УстановитьПараметр("Задание",Задание);
	Запрос.УстановитьПараметр("ИмяПеременной",Справочники.ИменаПеременных.РежимScreenShot);
	...
	|	И ПеременныеЗаданий.ЭтоПараметрНастройки = ИСТИНА";
	Запрос.УстановитьПараметр("Задание",Задание);
	Запрос.УстановитьПараметр("ИмяПеременной",Справочники.ИменаПеременных.АдресИнтернетExternAutomationUI);
...
Функция СоздатьНовоеЗаданиеНаСервере()
	...
	Попытка
		...
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.Задание = Задание;
		НоваяЗапись.ИмяПеременной = Справочники.ИменаПеременных.РежимScreenShot;
		НоваяЗапись.ЗначениеПеременной = РежимScreenShot;
		
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.Задание = Задание;
		НоваяЗапись.ИмяПеременной = Справочники.ИменаПеременных.АдресИнтернетExternAutomationUI;
