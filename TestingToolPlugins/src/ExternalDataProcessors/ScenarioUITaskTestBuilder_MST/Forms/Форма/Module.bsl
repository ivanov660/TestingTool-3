	...
	Попытка
		...
		Если СоздаватьИскатьДействияАвтоматически=Истина И НЕ ЗначениеЗаполнено(ШаблонКомандыВыполнитьТест) Тогда
			...
			Если НЕ ЗначениеЗаполнено(ШаблонКомандыВыполнитьТест) Тогда
				ШаблонКомандыВыполнитьТест = СоздатьШаблонКоманды(ИдентификаторКонструктораУниверсальный,"Шаблон команды выполнить сценарный UI тест (МСТ) + screenshot",КлючПоискаШаблона,ПолучитьТекстШаблонаКомандыВыполненияСценарногоТеста(ФорматФайлаОтчета),Версия);
...
Функция ПолучитьКлючПоискаШаблона(Знач Действие="")
	...
	Если Действие="ВыполнитьТест" ИЛИ Действие="ШаблонВыполненияТеста" Тогда
		Ключ = "МенеджерСценарногоТеста/СценарныйТест/GIT/Screenshot/"+ФорматФайлаОтчета+?(ЗначениеЗаполнено(Действие),"/"+Действие,"");
