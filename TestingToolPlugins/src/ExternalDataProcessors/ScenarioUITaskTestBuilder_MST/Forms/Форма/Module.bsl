...
Процедура НайтиПараметрыПоЗаданию()
	...
	Если Выборка.Следующий() Тогда
		АдресИнтернетExternalAutomationUI = Выборка.Значение;
	КонецЕсли;
...
Функция СоздатьНовоеЗаданиеНаСервере()
	...
	Попытка
		...
		НоваяЗапись.ЭтоПараметрНастройки = Ложь;
		НоваяЗапись.ЗначениеПеременной = АдресИнтернетExternalAutomationUI;		
...
Функция ПроверитьНаличиеПодобноегоЗадания(Знач Наименование,Знач TaskID,Знач Задание)   	
	...
	// ++ IS-3605 2026.09.10 СкворцовКС 
	// 1. Добавил псевдонимы полей, в т.ч. TaskID
	// -- IS-3605 2026.09.10 СкворцовКС 
	Запрос = новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Задания.Ссылка КАК Ссылка,
	|	Задания.ID КАК TaskID,
