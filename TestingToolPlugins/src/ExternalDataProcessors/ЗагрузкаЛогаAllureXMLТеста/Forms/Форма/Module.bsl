...
Функция ЗагрузитьОтчетВыполненияСценарногоТестированияФорматAllureXML(ПутьКФайлуОтчета,Сообщение="") Экспорт
	...
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		СообщитьОбОшибке("ЗагрузитьОтчетВыполненияСценарногоТестированияФорматAllureXML",ТекстОшибки);
		Возврат Ложь;	
	КонецПопытки;		
	...
	Исключение
		...
		СообщитьОбОшибке("ЗагрузитьОтчетВыполненияСценарногоТестированияФорматAllureXML",ТекстОшибки);
	КонецПопытки;
	//Для каждого стр из ПакетОбъект.test_cases.test_case Цикл
	//КонецЦикла;
	
	//НомерПроверки = Description.НомерПроверки;
	ВремяВыполнения = (ПакетОбъект.stop-ПакетОбъект.start)/1000;
	ДатаВыполенения = Дата('00010101000000')+ПакетОбъект.start/1000;
	//КоличествоТестов = Description.КоличествоТестов;
	//КоличествоПровалов = Description.КоличествоНеудач;
	//КоличествоОшибок = Description.КоличествоОшибок;
	//КоличествоПропущенных = Description.КоличествоПропущенных;
	//СтрокаПодключения = Description.СтрокаПодключения;
