...
&НаСервереБезКонтекста
Функция ПолучитьПроверкуДляТестируемогоКлиента(Знач ТестируемыйКлиент,Знач ТекущаяПроверка,Знач Направление="назад")
	...
	|	Т.Проверка.Код УБЫВ";
	Запрос.УстановитьПараметр("ТестируемыйКлиент",ТестируемыйКлиент);
	Запрос.УстановитьПараметр("ТекущаяПроверка",ТекущаяПроверка);
	
	Если Направление="назад" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"&Напраление","Т.Проверка<&ТекущаяПроверка");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"&Напраление","Т.Проверка>&ТекущаяПроверка");		
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"Т.Проверка.Код УБЫВ","Т.Проверка.Код ВОЗР");
	КонецЕсли;
...
Процедура Назад(Команда)
	мПроверка = ПолучитьПроверкуДляТестируемогоКлиента(Объект.ТестируемыйКлиент,Объект.Проверка,"назад");
	Если ЗначениеЗаполнено(мПроверка) Тогда
		Объект.Проверка = мПроверка;
		УстановитьОтборыСпискаТестов(Ложь);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура Вперед(Команда)
	мПроверка = ПолучитьПроверкуДляТестируемогоКлиента(Объект.ТестируемыйКлиент,Объект.Проверка,"вперед");
	Если ЗначениеЗаполнено(мПроверка) Тогда
		Объект.Проверка = мПроверка;
		УстановитьОтборыСпискаТестов(Ложь);
	КонецЕсли;
...
&НаКлиенте
Процедура ПустаяПроверка(Команда)
	Если Элементы.ПустаяПроверка.Пометка=Истина Тогда
		Элементы.ПустаяПроверка.Пометка=Ложь;
		УстановитьОтборыСпискаТестов();
	Иначе
		Элементы.ПустаяПроверка.Пометка=Истина;
		Объект.Проверка = Неопределено;
		УстановитьОтборыСпискаТестов(Ложь);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура УстановитьОтборыСпискаТестов(ИскатьПоследнююПроверку=Истина)
	...
	Если ИскатьПоследнююПроверку=Истина Тогда
...
Процедура УстановитьОтборыСпискаСлучаев()
	...
	СписокСлучаев.Параметры.УстановитьЗначениеПараметра("Проверка",Объект.Проверка);
	СписокСлучаев.Параметры.УстановитьЗначениеПараметра("ОтборПадения",ОтборПадения);
	СписокСлучаев.Параметры.УстановитьЗначениеПараметра("ОтборОшибки",ОтборОшибки);
	СписокСлучаев.Параметры.УстановитьЗначениеПараметра("ОтборПропуски",ОтборПропуски);
	СписокСлучаев.Параметры.УстановитьЗначениеПараметра("ОтборУспешные",ОтборУспешные);
...
Процедура УстановитьОтборыСпискаШагов()
	...
	СписокШагов.Параметры.УстановитьЗначениеПараметра("Проверка",Объект.Проверка);
	СписокШагов.Параметры.УстановитьЗначениеПараметра("ОтборПадения",ОтборПадения);
	СписокШагов.Параметры.УстановитьЗначениеПараметра("ОтборОшибки",ОтборОшибки);
	СписокШагов.Параметры.УстановитьЗначениеПараметра("ОтборПропуски",ОтборПропуски);
	СписокШагов.Параметры.УстановитьЗначениеПараметра("ОтборУспешные",ОтборУспешные);
