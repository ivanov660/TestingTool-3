...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИдентификаторКонструктора = "UnitCatalogTaskTestBuilder_xddTestRunner";
	...
	Иначе
		...
		Если ПредельноеВремяОжиданияВыполнения=0 Тогда
			ПредельноеВремяОжиданияВыполнения = 30;
		КонецЕсли;		
...
Процедура Вперед(Команда)
	...
	ИначеЕсли Элементы.СтраницыОбработки.ТекущаяСтраница=Элементы.СтраницаНастройкиВыполненияТеста Тогда
		Если НЕ ЗначениеЗаполнено(НаборТестов) Тогда
			Сообщить("Выберите набор тестов прежде!");
		Иначе
...
Функция ПараметрыУказанияПараметровЗаданияКорректны()
	...
	Если НЕ ЗначениеЗаполнено(ДействиеВыполнитьТест) Тогда
		Сообщить("Укажите действие 'Выполнить набор тестов'");
		Отказ = Истина;
	КонецЕсли;
...
Процедура НайтиПараметрыПоЗаданию()
	...
	Запрос.УстановитьПараметр("ИмяПеременной",Справочники.ИменаПеременных.НаборТестов);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Выборка.Следующий() Тогда
		НаборТестов = Выборка.Значение;
	КонецЕсли;
...
Процедура НайтиШаблоныДействия()
	...
	ШаблонКомандыВыполнитьТест = НайтиШаблонКоманды(ИдентификаторКонструктораУниверсальный,ПолучитьТекстШаблонаКомандыВыполненияЮнитТестаВсеИзКаталога(ФорматФайлаОтчета),КлючПоискаШаблона);
	...
	КлючПоискаШаблона = ПолучитьКлючПоискаШаблона("ВыполнитьНаборТестовИзКаталога");
...
Функция СоздатьНовоеЗаданиеНаСервере()
	...
	Попытка
		...
		Если СоздаватьИскатьДействияАвтоматически=Истина И НЕ ЗначениеЗаполнено(ШаблонКомандыВыполнитьТест) Тогда
			КлючПоискаШаблона = ПолучитьКлючПоискаШаблона("ВыполнитьНаборТестов"); 
			ШаблонКомандыВыполнитьТест = НайтиШаблонКоманды(ИдентификаторКонструктораУниверсальный,ПолучитьТекстШаблонаКомандыВыполненияЮнитТестаВсеИзКаталога(ФорматФайлаОтчета),КлючПоискаШаблона);
			Если НЕ ЗначениеЗаполнено(ШаблонКомандыВыполнитьТест) Тогда
				ШаблонКомандыВыполнитьТест = СоздатьШаблонКоманды(ИдентификаторКонструктораУниверсальный,"Шаблон команды выполнить набор юнит тестов из каталога",КлючПоискаШаблона,ПолучитьТекстШаблонаКомандыВыполненияЮнитТестаВсеИзКаталога(ФорматФайлаОтчета));
		...
		Если СоздаватьИскатьДействияАвтоматически=Истина И НЕ ЗначениеЗаполнено(ДействиеВыполнитьТест) Тогда
			КлючПоискаШаблона = ПолучитьКлючПоискаШаблона("ВыполнитьНаборТестовИзКаталога");
			ДействиеВыполнитьТест = СоздатьДействиеЗадания(ИдентификаторКонструктораУниверсальный,"Выполнить набор юнит тестов из каталога",КлючПоискаШаблона,ШаблонКомандыВыполнитьТест);
		...
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.Задание = Задание;
		НоваяЗапись.ИмяПеременной = "%НаборТестов%";
		НоваяЗапись.ЗначениеПеременной = НаборТестов;
		...
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.Задание = Задание;
		НоваяЗапись.ИмяПеременной = "%КаталогНабораТестов%";
		НоваяЗапись.ЗначениеПеременной = НаборТестов;
		НоваяЗапись.ИмяРеквизита = "ПутьКаталог";
		...
		СоздатьОбновитьПараметр(Задание,Справочники.ИменаПеременных.НаборТестов,НаборТестов);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("UnitCatalogTaskTestBuilder_xddTestRunner",УровеньЖурналаРегистрации.Ошибка,Неопределено,Неопределено,ТекстОшибки);
...
Функция СформироватьОписаниеСоздаваемогоЗдания()
	...
	Html = Html + "<b>Набор тестов:</b> <span color='blue'>"+НаборТестов+"</span></br>";
	...
	Html = Html + "<b>Действие запуска выполнения набора тестов:</b> <span color='blue'>"+ДействиеВыполнитьТест+"</span></br>";
...
Функция СоздатьДействиеЗадания(Знач ИдентификаторКонструктора, Знач Наименование, Знач КлючПоиска, Знач ШаблонКоманды, Знач МаксВремяОжидания=600)
	...
	ИначеЕсли Найти(КлючПоиска,"ВыполнитьНаборТестовИзКаталога") Тогда
...
&НаСервереБезКонтекста
Функция ПолучитьТекстШаблонаКомандыВыполненияЮнитТестаВсеИзКаталога(ФорматФайлаОтчета)
	Возврат """%ПутьКИсполняемомуФайлу1С%""  %СтрокаСоединения%  /UseHwLicenses- /DisableStartupMessages
	| /Execute ""%ПутьККаталогуGIT%\xddTestRunner.epf""  
	| /C""
	| xddRun ЗагрузчикКаталога """"%ПутьККаталогуGIT%\%КаталогНабораТестов%\"""";
	| xddReport "+?(ФорматФайлаОтчета="Allure","ГенераторОтчетаAllureXML","ГенераторОтчетаJUnitXML")+" """"%ПутьККаталогуОтчетовВыполненияТестов%\report-%НомерПроверки%_%НаборТестов%.xml""""; 
	| xddShutdown;
	|"" ";
КонецФункции
...
Функция ПолучитьКлючПоискаШаблона(Знач Действие="")
	...
	Если Действие="ВыполнитьТест" ИЛИ Действие="ШаблонВыполненияТеста" Тогда
		Ключ = "xddTestRunner/НаборЮнитТестовИзКаталога/GIT/"+ФорматФайлаОтчета+?(ЗначениеЗаполнено(Действие),"/"+Действие,"");
