...
Процедура ПолучитьПредставлениеПутьКФайлу()
	...
	Если Объект.ОсновнойВариантХраненияСценария=ПредопределенноеЗначение("Перечисление.ВариантыХраненияСценариев.ВоВнешнемФайлеGIT") Тогда
		...
		ПредставлениеПутьККаталогуБиблиотекиСценариев = ПутьКаталогGIT+"\"+Объект.ПутьККаталогуБиблиотекиСценариев;
	Иначе
		ПредставлениеПутьКФайлу = Объект.ПутьКФайлу;
		ПредставлениеПутьКДанным = Объект.ПутьКФайлуДанных;
		ПредставлениеПутьККаталогуБиблиотекиСценариев = Объект.ПутьККаталогуБиблиотекиСценариев;
	КонецЕсли;
...
Процедура СформироватьПутьКФайлуПоПредставлению(Отказ, ТекущийОбъект)
	...
	Если Объект.ОсновнойВариантХраненияСценария=ПредопределенноеЗначение("Перечисление.ВариантыХраненияСценариев.ВоВнешнемФайлеGIT") Тогда
		...
		Если Не Найти(Врег(ПредставлениеПутьКФайлу),Врег(ПутьКаталогGIT)) Тогда
			Сообщить("Указанный путь к файлу не хранится в локальном каталоге GIT/Общем каталоге '"+ПредставлениеПутьКФайлу+"'! Выберите другой вариант хранения, укажите другой путь.");
		...
		Если Не Найти(Врег(ПредставлениеПутьКДанным),Врег(ПутьКаталогGIT)) И ЗначениеЗаполнено(ПредставлениеПутьКДанным) Тогда
			Сообщить("Указанный путь к файлу не хранится в локальном каталоге GIT/Общем каталоге '"+ПредставлениеПутьКДанным+"'! Выберите другой вариант хранения, укажите другой путь.");
			Сообщить("Путь к каталогу гит для текущего пользователя на текущем компьютере - '"+ПутьКаталогGIT+"'");
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		
		Если Не Найти(Врег(ПредставлениеПутьККаталогуБиблиотекиСценариев),Врег(ПутьКаталогGIT)) И ЗначениеЗаполнено(ПредставлениеПутьККаталогуБиблиотекиСценариев) Тогда
			Сообщить("Указанный путь к библиотеке не находится в локальном каталоге GIT/Общем каталоге '"+ПредставлениеПутьККаталогуБиблиотекиСценариев+"'! Выберите другой вариант хранения, укажите другой путь.");
			Сообщить("Путь к каталогу гит для текущего пользователя на текущем компьютере - '"+ПутьКаталогGIT+"'");
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		ПутьКДанным = СтрЗаменить(Врег(ПредставлениеПутьККаталогуБиблиотекиСценариев),Врег(ПутьКаталогGIT),"");
		ТекущийОбъект.ПутьККаталогуБиблиотекиСценариев = Прав(ПредставлениеПутьККаталогуБиблиотекиСценариев,СтрДлина(ПутьКДанным)-1);		
	Иначе
		ТекущийОбъект.ПутьКФайлу = ПредставлениеПутьКФайлу;
		ТекущийОбъект.ПутьКФайлуДанных = ПредставлениеПутьКДанным;
		ТекущийОбъект.ПутьККаталогуБиблиотекиСценариев = ПредставлениеПутьККаталогуБиблиотекиСценариев;
	КонецЕсли;
...
&НаКлиенте
Процедура ПредставлениеПутьККаталогуБиблиотекиСценариевНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога); 
	Диалог.Заголовок = "Выберите кталог библиотеки сценария"; 
	Диалог.ПолноеИмяФайла = ""; 
	Диалог.Каталог = ПредставлениеПутьККаталогуБиблиотекиСценариев;
	Диалог.МножественныйВыбор = Ложь; 
	ВыборФайлаОткрытияКаталогаБиблиотеки = новый ОписаниеОповещения("ВыборФайлаОткрытияКаталогаБиблиотеки",ЭтотОбъект);
	Диалог.Показать(ВыборФайлаОткрытияКаталогаБиблиотеки);

КонецПроцедуры

&НаКлиенте
Процедура ВыборФайлаОткрытияКаталогаБиблиотеки(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныеФайлы <> Неопределено И ВыбранныеФайлы.Количество() > 0 Тогда
		ПредставлениеПутьККаталогуБиблиотекиСценариев = ВыбранныеФайлы[0]; 
	КонецЕсли; 
