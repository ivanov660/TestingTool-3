...
Функция ОбработатьСтрокуПоПараметрам(Знач ИсходнаяСтрока, Знач СтруктураПараметров) Экспорт
	...
	Попытка
		СоответсвиеНастроек = ПолучитьСоответствиеНастроекПользователя(ПользователиКлиентСервер.ТекущийПользователь(),ПользователиКлиентСервер.ТекущееРабочееМесто());
	Исключение
		СоответсвиеНастроек = ПолучитьСоответствиеНастроекПользователя(ПользователиКлиентСервер.ТекущийПользователь(),ПользователиКлиентСервер.ТекущееРабочееМесто());
	КонецПопытки;
